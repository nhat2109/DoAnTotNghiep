<div class="col-md-3 col-sm-3 col-xs-6 no-padding product_4s">
    <div class="products-view-grid">
        <div class="product-item">
            <div class="product-item-container grid-view-item">
                <div class="left-block product-image-container product-image">
                    <a class="grid-view-item__link image-ajax" href="/product/{link}.html" title="{tieu_de}">
                        <img class="first-img img-responsive center-block" src="/thumbnail.php?w=320&img={minh_hoa}" data-src="/thumbnail.php?w=320&img={minh_hoa}" alt="{tieu_de}" />
                    </a>
                    {label_sale}
                    {icon_label}
                    <div class="button-link">
                        <a href="/product/{link}.html">
                            <button class="btn_df tt-btn-addtocart btn-buy btn-cart btn btn-gray left-to" sp_id="{id}" loai="{loai}" title="Xem chi tiết">
                                <i class="ion ion-md-basket"></i> Chi tiết
                            </button>
                        </a>
                    </div>
                </div>
                <div class="right-block">
                    <h4 class="title-product">
                        <a class="product-name line-clamp" href="/product/{link}.html" title="{tieu_de}">{tieu_de}</a>
                    </h4>
                    <div class="price">
                        <span class="price-old">{gia_cu}₫</span>
                        <span class="price-new">{gia_moi}₫</span>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>