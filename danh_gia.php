...
$thongtin_diem = mysqli_query($conn, "SELECT *,count(*) AS total FROM diem WHERE user_id='$user_id'");
$r_diem = mysqli_fetch_assoc($thongtin_diem);
if ($r_diem['total'] > 0) {
	$diem = number_format($r_diem['diem']);
} else {
	$diem = 0;
}
$list_danhmuc_top = json_decode($class_index->list_category_danhmuc_top($conn), true);
$tach_danhgia = json_decode($class_index->list_danhgia($conn, $user_id, $page, 10), true);
...
$replace = array(
	...
	'diem' => $diem,
	'list_danhgia' => $tach_danhgia['list'],
...
echo $skin->skin_replace('skin/danh_gia', $replace);
