    ...
    <script>
        $(document).ready(function () {
            ...
            function updateTotal() {
                ...
                $('.button_thanhtoan').text("Thanh toán " + formatCurrency(tong));
            }
        /*
            function saveShopFooterData(shopId, tinh, huyen, trongluong, subtotal, feeShip) {
                $.ajax({
                    url: '/process.php',
                    method: 'POST',
                    data: {
                        action: 'save_shop_footer',
                        shop_id: shopId,
                        tinh: tinh,
                        huyen: huyen,
                    ...
                    success: function (response) {
                        console.log('Saved shop footer data:', response);
                    },
                    error: function (xhr) {
                        console.error('Error saving shop footer data:', xhr.responseText);
                    }
                });
            }
        */
            function getFeeShipForAllShops() {
                ...
                $('.shopcart-shop-shipping').each(function () {
                    ...
                    const shopId = $footer.data('shop-id') || 0;
                    ...
                    const amount = parseInt($shippingSpan.data('subtotal').replace(/[.,]/g, '')) || 0;
                        ...
                        success: function (response) {
                            try {
                                ...
                                if (info.ok == 1) {
                                    ...
                                    // Lưu dữ liệu vào bảng shopcart_shop_footer
                                   // saveShopFooterData(shopId, sender_province, sender_district, weight, amount, fee);
                                } else {
                                    $shippingSpan.find('b').text('0 đ');
                                }
                                totalFeeShip += fee;
