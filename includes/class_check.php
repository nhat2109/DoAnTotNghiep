 ...
 class class_check extends class_manage{
    function remove_blank_line($text) {
        // Biểu thức chính quy để tìm các thẻ <p> hoặc <div> trống hoặc chỉ chứa <br>, &nbsp; hoặc khoảng trắng
        $emptyTagPattern = '/<(p|div)>(\s*|(&nbsp;)*|(<br\s*\/?>\s*)*)<\/\1>/i';
        // Biểu thức chính quy để tìm các thẻ <br> trống
        $emptyBrPattern = '/^\s*<br\s*\/?>\s*$/i';
        ...
        foreach ($lines as $line) {
            // Kiểm tra nếu dòng chỉ chứa thẻ <p> trống hoặc chỉ chứa <br>, &nbsp;
            if (preg_match($emptyTagPattern, $line) && strpos($line, '<p>') !== false) {
                $emptyParagraphCount++;
            } else if (preg_match($emptyTagPattern, $line) && strpos($line, '<div>') !== false) {
                $emptyDivCount++;
            } else {
                $emptyParagraphCount = 0;
                $emptyDivCount = 0;
            ...
            // Kiểm tra nếu dòng chỉ chứa <br>
            if (preg_match($emptyBrPattern, $line)) {
                $emptyBrCount++;
            } else {
                $emptyBrCount = 0;
            }

            // Xóa nội dung bên trong nếu chỉ chứa <br> hoặc &nbsp; và không phải là dòng trống thứ 2
            if ($emptyParagraphCount == 1 && preg_match('/<(p)>(\s*<br\s*\/?>\s*)<\/\1>/i', $line)) {
                $line = preg_replace('/(\s*<br\s*\/?>\s*)/', '', $line);
            }
            if ($emptyDivCount == 1 && preg_match('/<(div)>(\s*<br\s*\/?>\s*)<\/\1>/i', $line)) {
                $line = preg_replace('/(\s*<br\s*\/?>\s*)/', '', $line);
            }

            // Chỉ thêm dòng vào kết quả nếu nó không phải là dòng trống thứ 2 cho <p>, <div> hoặc dòng trống thứ 3 cho <br>
            if ($emptyParagraphCount < 2 && $emptyDivCount < 2 && $emptyBrCount < 3) {
                $resultLines[] = $line;
            }
        }

        // Ghép lại các dòng thành văn bản hoàn chỉnh
        return implode("\n", $resultLines);
