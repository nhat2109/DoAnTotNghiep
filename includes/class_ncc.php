...
class class_ncc extends class_manage {
	...
	function list_donhang_socdo($conn, $user_id, $page, $limit) {
		...
		$start = $page * $limit - $limit;
		$i = $start;
		$thongtin = mysqli_query($conn, "SELECT * FROM donhang WHERE shop_id='$user_id' ORDER BY date_post DESC LIMIT $start,$limit");
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			$r_tt['i'] = $i;
			$r_tt['tongtien'] = number_format($r_tt['tongtien']) . 'đ';
...
///////////////////
	...
	//////////////////////////
	function list_donhang_ncc($conn, $user_id, $search, $status, $date_from, $date_to, $page, $limit, $valid_order_ids = [])
	{
		$skin = $this->load('class_skin_cpanel');
		$check = $this->load('class_check');
		$i = 0;
		...
		// Chỉ lấy các đơn hàng có ID nằm trong danh sách hợp lệ
		if (!empty($valid_order_ids)) {
			$valid_order_ids_str = implode(',', array_map('intval', $valid_order_ids)); // Đảm bảo các ID là số nguyên
			$where .= " AND id IN ($valid_order_ids_str)";
		} else {
			// Nếu không có đơn hàng hợp lệ, trả về rỗng
			return '';
		}
		...
		// Truy vấn chính
		$query = "SELECT * FROM donhang WHERE $where ORDER BY date_post DESC LIMIT $offset, $limit";
		...
		while ($r_tt = mysqli_fetch_assoc($result)) {
			...
			$r_tt['tongtien'] = 0;
			$r_tt['date_post'] = date('d/m/Y', $r_tt['date_post']);
			$r_tt['id'] = $r_tt['id']; // Đảm bảo id được truyền vào template
			$r_tt['ma_don'] = $r_tt['ma_don']; // Đảm bảo ma_don được truyền vào template
			if ($r_tt['status'] == 1) {
				$r_tt['status'] = 'Đã tiếp nhận đơn';
				$r_tt['huy'] = '';
			} else if ($r_tt['status'] == 2) {
			...
			foreach ($tach_sanpham as $variant_id => $value) {
				...
				if ($result_sp && mysqli_num_rows($result_sp) > 0) {
					...
					if ($sp_user_id == $user_id) {
						// Lấy thông tin sản phẩm và thành tiền
						$tieu_de = $value['tieu_de'];
						$color = $value['color'] ? ' - Màu: ' . $value['color'] : '';
						$size = $value['size'] ? ' - Size: ' . $value['size'] : '';
						$thanhtien = number_format(str_replace(',', '', $value['thanhtien'])) . 'đ'; // Loại bỏ dấu phẩy và định dạng lại
						$r_tt['tongtien'] += str_replace(',', '', $value['thanhtien']); // Cộng dồn thành tiền
						$list_sanpham .= "+ $tieu_de$color$size (Thành tiền: $thanhtien)<br>";
					}
				}
			}
			$r_tt['list_sanpham'] = $list_sanpham ?: 'Không có sản phẩm hợp lệ'; // Đảm bảo có giá trị mặc định
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_donhang_shop', $r_tt);
		}
