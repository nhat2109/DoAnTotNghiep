...
class class_ncc extends class_manage
{
	...
	function list_menu($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_menu', $r_tt);
		}
		return $list;
	}
	function list_kq_timkiem_thanhviennhom($conn, $page, $limit, $key, $user_id)
	{
		...
		$query = "SELECT u.*, 
					...
					(SELECT count(*) FROM donhang_ncc d_ncc WHERE d_ncc.user_id = u.user_id AND d_ncc.status NOT IN ('3', '4')) AS total_donhang_ncc,
					(SELECT SUM(tongtien) FROM donhang_ncc d_ncc WHERE d_ncc.user_id = u.user_id AND d_ncc.status NOT IN ('3', '4')) AS total_doanhso_ncc,
		...
		while ($r_tt = $result->fetch_assoc()) {
			...
			$tong_doanhso = $r_tt['total_doanhso'] + $r_tt['total_doanhso_ncc'];
			$total_donhang = $r_tt['total_donhang'] + $r_tt['total_donhang_ncc'];
				...
				if ($createdDate >= $cutoffDate) {
					...
					if ($r_tt['ncc'] == 1) {
						...
						elseif ($currentDate <= $expirationDate) {
							...
							$r_tt['ncc'] = 4; // Tạm khóa
							mysqli_query($conn, "UPDATE user_info SET ncc = 4 WHERE user_id='{$r_tt['user_id']}'");
						}
					}

					// Hiển thị radio button tương ứng với trạng thái
					//<input type="radio" value="1" name="drop_' . $r_tt['user_id'] . '" ' . ($r_tt['ncc'] == 1 ? 'checked' : '') . '> Đã duyệt 
					$r_tt['tinh_trang'] = '
					<input type="radio" name="drop_' . $r_tt['user_id'] . '" value="2" ' . ($r_tt['ncc'] == 2 ? 'checked' : '') . '> Dùng thử 
					<input type="radio" value="1" name="drop_' . $r_tt['user_id'] . '" ' . ($r_tt['ncc'] == 1 || $r_tt['leader'] == 1 ? 'checked' : '') . '> Đã duyệt
					<input type="radio" value="4" name="drop_' . $r_tt['user_id'] . '" ' . ($r_tt['ncc'] == 4 ? 'checked' : '') . '> Tạm khóa';
				} else {
					if ($r_tt['ncc'] == 2) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_thanhvien_nhom', $r_tt);
		}
	...
	function list_notification($conn, $user_id, $loai, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/li_notification', $r_tt);
		}
		$info = array(
			'total' => $total,
			'list' => $list
	...
	////////////////////
	// public function list_phanloai_color($conn, $sp_id, $sub_product = '')
	// 	{
	// 		$query = "SELECT DISTINCT color, ten_color, ma_mau FROM phanloai_sanpham_shop WHERE sp_id = ? AND kho_sanpham_shop > 0";
	// 		$stmt = $conn->prepare($query);
	...
	// 			$i++;
	// 			if ($i == 1) {
	// 				$ten_color = $row['ten_color']; // Lấy tên màu đầu tiên làm mặc định
	// 			}
	// 			// Tạo HTML cho danh sách màu
	// 			$list_color .= '
	//         <button type="button" class="li_color" sp_id="' . $sp_id . '" color="' . $row['color'] . '" tieu_de="' . htmlspecialchars($row['ten_color']) . '">
	//             ' . htmlspecialchars($row['ten_color']) . '
	//         </button>';
	// 		}

	// 		if (empty($list_color)) {
	// 			$list_color = '<div>Không có màu nào</div>';
	// 		}
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if (intval($r_tt['id_lichsu']) > 0) {
				if ($r_tt['hoan_thanh'] == 1) {
					$list .= $skin->skin_replace('skin_ncc/box_action/li_nhiemvu_hoanthanh', $r_tt);
				} else {
					...
					$list .= $skin->skin_replace('skin_ncc/box_action/li_nhiemvu_mo', $r_tt);
					unset($list_noidung);
				}
			} else {
				$list .= $skin->skin_replace('skin_ncc/box_action/li_nhiemvu_moi', $r_tt);
			}
		}
		return $list;
	}
	...
	function list_bom($conn, $user_id, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if ($r_tt['user_id'] == 0) {
				...
				$r_tt['action'] = '<a href="/ncc/edit-bom?id=' . $r_tt['id'] . '" class="edit">Sửa</a><a href="javascript:;" onclick="confirm_del(\'del\',\'bom\', \'Xác nhận xóa bom hàng\', \'' . $r_tt['id'] . '\');" class="del">Xóa</a>';
			} else {
				$r_tt['action'] = '';
			}
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_bom', $r_tt);
		}
		return $list;
	}
	function creat_random($conn, $loai)
	{
		...
		} else if ($loai == 'donhang_ncc') {
			$string = $check->random_number(6);
			$thongtin = mysqli_query($conn, "SELECT *,count(*) AS total FROM donhang_ncc WHERE ma_don='$string'");
			$r_tt = mysqli_fetch_assoc($thongtin);
			if ($r_tt['total'] > 0) {
				$this->creat_random($conn, $loai);
			} else {
	...
	function list_yeucau($conn, $thanh_vien, $phien)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/li_yeucau', $r_tt);
		}
		return $list;
	}
	////////////////////
			...
			foreach ($list_x as $key => $value) {
				...
				if ($value['user_out'] == $_SESSION['user_end']) {
					if ($user_id == $_SESSION['user_end']) {
						$list .= $skin->skin_replace('skin_ncc/box_action/li_chat_right', $value);
					} else {
						$list .= $skin->skin_replace('skin_ncc/box_action/li_chat_left', $value);
					}
				} else {
					if ($value['user_out'] == $user_id) {
						$list .= $skin->skin_replace('skin_ncc/box_action/li_chat_right_avatar', $value);
					} else {
						$list .= $skin->skin_replace('skin_ncc/box_action/li_chat_left_avatar', $value);
					}
				}
	...
	function list_donhang_nhom($conn, $list_id, $loai, $status, $total, $page, $limit)
	{
		...
		if ($loai == 'drop') {
			...
			while ($r_tt = mysqli_fetch_assoc($thongtin)) {
				...
				$list .= $skin->skin_replace('skin_ncc/box_action/li_don_nhom', $r_tt);
			}
		} else if ($loai == 'socdo') {
			$thongtin_ncc = mysqli_query($conn, "SELECT donhang_ncc.*,user_info.name,user_info.username FROM donhang_ncc INNER JOIN user_info ON user_info.user_id=donhang_ncc.user_id WHERE donhang_ncc.user_id IN ($list_id) AND donhang_ncc.status='$status' ORDER BY donhang_ncc.id DESC LIMIT $start,$limit");
			while ($r_tt = mysqli_fetch_assoc($thongtin_ncc)) {
				$i--;
				$r_tt['i'] = $i;
				$r_tt['ngay'] = date('d/m/Y', $r_tt['date_post']);
				$r_tt['hoahong'] = number_format(($r_tt['tamtinh'] / 100) * 5);
				...
				if ($r_tt['status'] == 0) {
					...
					$r_tt['status'] = 'Giao thành công';
					$thongtin_hh = mysqli_query($conn, "SELECT * FROM hoahong_nhom WHERE ma_don='{$r_tt['ma_don']}' AND loai_don='ncc'");
					$total_hh = mysqli_num_rows($thongtin_hh);
					if ($total_hh == 0) {
						$r_tt['thanhtoan_hh'] = 'Chưa thanh toán';
					} else {
	...
	///////////////////
	function list_ncc($conn, $user_id, $page, $limit)
	{
		$skin = $this->load('class_skin_cpanel');
		$check = $this->load('class_check');
		$start = $page * $limit - $limit;
		$i = $start;
		$thongtin = mysqli_query($conn, "SELECT * FROM user_info WHERE aff='$user_id' AND ctv='1' ORDER BY user_id ASC LIMIT $start, $limit");
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			$r_tt['i'] = $i;
			$thongtin_kichhoat = mysqli_query($conn, "SELECT * FROM user_gh WHERE user_id='{$r_tt['user_id']}'");
			$total_kichhoat = mysqli_num_rows($thongtin_kichhoat);
			if ($total_kichhoat > 0) {
				$r_tt['kichhoat'] = 'Đã kích hoạt';
			} else {
				$r_tt['kichhoat'] = 'Chưa kích hoạt';
			}
			$r_tt['created'] = date('d/m/Y', $r_tt['created']);
			$list .= $skin->skin_replace('skin_dropship/box_action/tr_tuyendung_ncc', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_thanhvien_nhom($conn, $user_id, $page, $limit)
	{
		...
		(SELECT count(*) FROM donhang_ncc d_ncc WHERE d_ncc.user_id=u.user_id AND d_ncc.status!='3' AND d_ncc.status!='4') AS total_donhang_ncc,
		(SELECT sum(tongtien) FROM donhang_ncc d_ncc WHERE d_ncc.user_id=u.user_id AND d_ncc.status!='3' AND d_ncc.status!='4') AS total_doanhso_ncc,
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if ($r_tt['ncc'] == 0) {
				...
				// Kiểm tra điều kiện "Đã duyệt" (ncc = 1)
				...
				$r_tt['tinh_trang'] = '
					<input type="radio" name="drop_' . $r_tt['user_id'] . '" value="2" ' . ($r_tt['ncc'] == 2 ? 'checked' : '') . '> Dùng thử 
					<input type="radio" name="drop_' . $r_tt['user_id'] . '" value="1" ' . ($r_tt['ncc'] == 1 ? 'checked' : '') . '> Kích hoạt
					<input type="radio" name="drop_' . $r_tt['user_id'] . '" value="4" ' . ($r_tt['ncc'] == 4 ? 'checked' : '') . '> Tạm khóa';
			}
	...
	function thongke_doanhthu($conn, $list_id, $dau, $cuoi)
	{
		...
		$check = $this->load('class_check');
		$start = $page * $limit - $limit;
		$thongtin = mysqli_query($conn, "SELECT * FROM donhang_shop WHERE shop IN ($list_id) AND date_post>='$dau' AND date_post<='$cuoi'");
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$ngay = date('d', $r_tt['date_post']);
			$ngay = intval($ngay);
			if ($r_tt['status'] == 7) {
	...
	function thongke_hoahong($conn, $list_leader, $list_id, $list_all, $dau, $cuoi)
	{
		...
		$thongtin_ncc = mysqli_query($conn, "SELECT * FROM donhang_ncc WHERE user_id IN ($list_id) AND date_post>='$dau' AND date_post<='$cuoi'");
		while ($r_tt_ncc = mysqli_fetch_assoc($thongtin_ncc)) {
			$ngay = date('d', $r_tt_ncc['date_post']);
			$ngay = intval($ngay);
			if ($r_tt_ncc['status'] == 5) {
				$donhang_ncc_nhom++;
				$doanhthu_ncc_nhom += $r_tt_ncc['tamtinh'];
			}
		}
		$doanhthu_ncc_nhom = intval(($doanhthu_ncc_nhom / 100) * 5);
		...
		$donhang_tong = $donhang_nhom + $donhang_ncc_nhom + $donhang_nhom_gioithieu;
		$doanhthu_tong = ($doanhthu_nangcap / 2) + $hoahong_gioithieu + $doanhthu_nhom + $doanhthu_ncc_nhom;
		$info = array(
			'donhang_nangcap' => $donhang_nangcap,
			'doanhthu_nangcap' => $doanhthu_nangcap / 2,
			'donhang_nhom' => $donhang_nhom + $donhang_ncc_nhom,
			'doanhthu_nhom' => $doanhthu_nhom + $doanhthu_ncc_nhom,
	...
	function thongke_donhang_nhom_nam($conn, $thanhvien, $dau, $cuoi)
	{
		...
		$thongtin_ncc = mysqli_query($conn, "SELECT * FROM donhang_ncc WHERE date_post>='$dau' AND date_post<='$cuoi' AND user_id IN ($thanhvien)");
		while ($r_tt_ncc = mysqli_fetch_assoc($thongtin_ncc)) {
			$month = date('m', $r_tt_ncc['date_post']);
			$month = intval($month);
			if ($month == 1) {
				$month_ncc_1++;
			} else if ($month == 2) {
		...
		$month_1 = $month_1 + $month_ncc_1;
		$month_2 = $month_2 + $month_ncc_2;
		$month_3 = $month_3 + $month_ncc_3;
		$month_4 = $month_4 + $month_ncc_4;
		$month_5 = $month_5 + $month_ncc_5;
		$month_6 = $month_6 + $month_ncc_6;
		$month_7 = $month_7 + $month_ncc_7;
		$month_8 = $month_8 + $month_ncc_8;
		$month_9 = $month_9 + $month_ncc_9;
		$month_10 = $month_10 + $month_ncc_10;
		$month_11 = $month_11 + $month_ncc_11;
		$month_12 = $month_12 + $month_ncc_12;
	...
	function thongke_donhang_nhom_thang($conn, $thanhvien, $thang, $nam, $dau, $cuoi)
	{
		...
		if ($thang == 2) {
			if (checkdate(02, 29, $nam) == true) {
				for ($i = 1; $i <= 29; $i++) {
					$data_thang .= intval($data_ngay[$i] + $data_ngay_ncc[$i]) . ',';
				}
			} else {
				for ($i = 1; $i <= 28; $i++) {
					$data_thang .= intval($data_ngay[$i] + $data_ngay_ncc[$i]) . ',';
	...
	function thongke_donhang_nhom_tuan($conn, $thanhvien, $dau, $cuoi)
	{
		...
		while ($r_tt_ncc = mysqli_fetch_assoc($thongtin_ncc)) {
			$month = date('m', $r_tt_ncc['date_post']);
			$day = date('d', $r_tt_ncc['date_post']);
			$year = date('Y', $r_tt_ncc['date_post']);
			$wkday = date('l', mktime('0', '0', '0', $month, $day, $year));
			if ($wkday == 'Monday') {
				$mon_ncc++;
			} else if ($wkday == 'Tuesday') {
		...
		return intval($mon + $mon_ncc) . ',' . intval($tus + $tus_ncc) . ',' . intval($web + $web_ncc) . ',' . intval($thu + $thu_ncc) . ',' . intval($fri + $fri_ncc) . ',' . intval($sat + $sat_ncc) . ',' . intval($sun + $sun_ncc);
	}
	///////////////////
	...
	function list_banner_qc($conn, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$i++;
			$r_tt['i'] = $i;
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_banner_qc', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_slide($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$i++;
			$r_tt['i'] = $i;
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_slide', $r_tt);
		}
		return $list;
	}
	///////////////////
	function list_share_sanpham($conn, $id)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if ($check->is_mobile() == true) {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_share_sanpham_mobile', $r_tt);
			} else {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_share_sanpham_laptop', $r_tt);
			}
			unset($list_anh);
		}
		$info = array(
	...
	////////////////////
	function list_noidung_ban_sanpham($conn, $id, $user_id, $page, $limit)
	{
		$skin = $this->load('class_skin');
		$check = $this->load('class_check');
		$start = $page * $limit - $limit;

		$thongtin = mysqli_query($conn, "SELECT * FROM list_share_sanpham WHERE sp_id='$id' AND shop_id='$user_id' ORDER BY id DESC LIMIT $start,$limit");
		$i = $start;
		$list = "";
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if (!empty($data['minh_hoa'])) {
				$tach_anh = explode(',', $data['minh_hoa']);
				$data['minh_hoa'] = $tach_anh[0];
				$duoi = $check->duoi_file($data['minh_hoa']);
				if (in_array($duoi, array('mp4', 'wmv', 'mov')) == true) {
					$data['minh_hoa'] = '/images/video.jpg';
				}
			}

			// Thêm ../ vào đường dẫn để đi ra thư mục gốc
			$list .= $skin->skin_replace('../skin_ncc/box_action/tr_noidung_bansp', $data);
		}

		if ($list == "") {
			$list = '<tr><td colspan="4" style="text-align: center;">Chưa có nội dung bán hàng nào</td></tr>';
		}
	...
	function list_giaodien($conn, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_giaodien', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_video_category($conn, $id, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			$r_tt['i'] = $i;
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_video', $r_tt);
		}
		return $list;
	}
	////////////////////
	...
	function list_idol($conn, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			$r_tt['i'] = $i;
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_idol', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_dat_live($conn, $user_id, $total, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_livestream', $r_tt);
		}
		return $list;
	}
	////////////////////
	...
	function list_chitieu($conn, $user_id, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_chitieu', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_naptien($conn, $user_id, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if ($r_tt['status'] == 1) {
				...
				$r_tt['status'] = 'Chờ xử lý';
				$r_tt['hanhdong'] = '<a href="/ncc/edit-naptien?id=' . $r_tt['id'] . '" class="edit">chi tiết</a>';
			}
			$r_tt['noidung'] = 'naptien ' . $r_tt['username'] . ' ' . $r_tt['id'];
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_naptien', $r_tt);
		}
		return $list;
	}
	////////////////////
	...
	function list_ruttien($conn, $user_id, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_ruttien', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_goi_seeding($conn, $loai)
	{
		...
		$start = $page * $limit - $limit;
		$i = $start;
		$thongtin = mysqli_query($conn, "SELECT * FROM seeding_shopee_ncc WHERE loai='$loai' ORDER BY thu_tu ASC");
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			if ($i % 2 == 0) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_goi_dichvu', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_coupon($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_coupon', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_deal($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_deal', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_flash_sale($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_flash_sale', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_donhang_affiliate($conn, $user_id, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_donhang_aff', $r_tt);
			unset($list_sanpham);
			unset($tong_hoahong);
		}
		return $list;
	...
	////////////////////
	function list_donhang_socdo($conn, $user_id, $search = '', $status = 0, $date_from = '', $date_to = '', $page = 1, $limit = 20)
	{
		$skin = $this->load('class_skin_cpanel');
		$check = $this->load('class_check');
		$start = $page * $limit - $limit;
		$i = $start;
		$query = "SELECT * FROM donhang WHERE shop_id='$user_id'";
		if (!empty($search)) {
			$search = mysqli_real_escape_string($conn, $search);
			$query .= " AND (id LIKE '%$search%' OR ma_don LIKE '%$search%')";
		}
		if ($status > 0) {
		...
		if (!empty($date_from) && !empty($date_to)) {
			...
			$query .= " AND date_post BETWEEN $from AND $to";
		}
		$query .= " ORDER BY date_post DESC LIMIT $start, $limit";
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			foreach ($tach_sanpham as $key => $value) {
				$s++;
				$size = !empty($value['size']) ? '<span style="font-size: 0.9em; color: #666;"> - Size: ' . strtoupper($value['size']) . '</span>' : '';
				$color = !empty($value['color']) ? '<span style="font-size: 0.9em; color: #666;"> - Màu: ' . $value['color'] . '</span>' : '';
				$list_sanpham .= '+ <strong>' . $value['tieu_de'] . '</strong><br>' . $color . '<br>' . $size . '<br>';
			}
			$r_tt['list_sanpham'] = $list_sanpham;
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_donhang_socdo', $r_tt);
		}
	...
	function list_donhang_drop($conn, $user_id, $page, $limit)
	{
		...
		$start = $page * $limit - $limit;
		$i = $start;
		$thongtin = mysqli_query($conn, "SELECT * FROM donhang WHERE user_id='$user_id' AND ncc='1' ORDER BY date_post DESC LIMIT $start,$limit");
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			$r_tt['i'] = $i;
			$r_tt['tongtien'] = number_format($r_tt['tongtien']) . 'đ';
			...
			$r_tt['list_sanpham'] = $list_sanpham;
			unset($list_sanpham);
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_donhang_drop', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_donhang_moi($conn, $user_id, $page, $limit)
	{
		...
		$time_now = time();
		$time_week_ago = $time_now - (7 * 24 * 60 * 60);
		...
		// === ĐẾM SỐ LƯỢNG ĐƠN HÀNG TRƯỚC ===
		if ($user_id == '') {
			$count_query = "SELECT COUNT(*) AS total FROM donhang_shop WHERE date_post >= '$time_week_ago'";
		} else {
			$count_query = "SELECT COUNT(*) AS total FROM donhang_shop WHERE shop = '$user_id' AND date_post >= '$time_week_ago'";
		}
		$count_result = mysqli_query($conn, $count_query);
		$count_data = mysqli_fetch_assoc($count_result);
		$total = (int) $count_data['total'];
		if ($total === 0) {
			$list = '<p><center>Chưa có đơn nào trong tuần, cần cố gắng hơn!</center></p>';
		}
		// === CHỈ TRUY VẤN CHI TIẾT KHI ĐÃ CÓ ĐƠN ===
		if ($user_id == '') {
			$query = "SELECT donhang_shop.*, user_info.avatar 
					  FROM donhang_shop 
					  LEFT JOIN user_info ON donhang_shop.user_id = user_info.user_id 
					  WHERE donhang_shop.date_post >= '$time_week_ago' 
					  ORDER BY donhang_shop.date_post ASC 
					  LIMIT $start, $limit";
		} else {
			$query = "SELECT donhang_shop.*, user_info.avatar 
					  FROM donhang_shop 
					  LEFT JOIN user_info ON donhang_shop.user_id = user_info.user_id 
					  WHERE donhang_shop.shop = '$user_id' 
					  AND donhang_shop.date_post >= '$time_week_ago' 
					  ORDER BY donhang_shop.date_post ASC 
					  LIMIT $start, $limit";
		}
		$thongtin = mysqli_query($conn, $query);
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$r_tt['tong_tien'] = number_format($r_tt['tongtien']) . 'đ';
			$r_tt['date_post'] = $check->chat_update($r_tt['date_post']);
			...
			if ($r_tt['user_id'] > 0) {
				...
				if (!isset($sodon[$u]))
					$sodon[$u] = 0;
				$sodon[$u]++;
				$r_tt['noi_dung'] = ($sodon[$u] > 1)
					? 'Vừa có <b>đơn hàng thứ ' . $sodon[$u] . '</b> trong tuần'
					: 'Vừa có đơn hàng';
			} else {
				$r_tt['noi_dung'] = 'Vừa có đơn hàng';
			}
			$list_cu_slide[] = $skin->skin_replace('skin_ncc/box_action/li_donhang_slide', $r_tt);
			$list_cu[] = $skin->skin_replace('skin_ncc/box_action/li_donhang', $r_tt);
		}

		$list = implode('', array_reverse($list_cu));
		$list_slide = implode('', array_reverse($list_cu_slide));
	...
	function list_donhang($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$r_tt['list_sanpham'] = $list_sanpham;
			unset($list_sanpham);
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_donhang_shop', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_category($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_category', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_category_sanpham($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_category_sanpham', $r_tt);
		}
		return $list;
	}
	///////////////////
	...
	function list_div_main_category_sanpham_ncc($conn, $category)
	{
		$tach_category = explode(',', $category);
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$total++;
			if (in_array($r_tt['cat_id'], $tach_category) == true) {
				$list .= '<div class="li_input" id="input_' . $r_tt['cat_id'] . '"><input type="checkbox" name="category_ncc[]" value="' . $r_tt['cat_id'] . '" checked> ' . $r_tt['cat_tieude'] . '</div>';
				$list_id .= $r_tt['cat_id'] . ',';
			} else {
				$list .= '<div class="li_input" id="input_' . $r_tt['cat_id'] . '"><input type="checkbox" name="category_ncc[]" value="' . $r_tt['cat_id'] . '"> ' . $r_tt['cat_tieude'] . '</div>';
			}
		}
		$list_id = substr($list_id, 0, -1);
		return json_encode(array('list' => $list, 'list_id' => $list_id, 'total' => $total));
	...
	//////////////////////////////////
	function list_div_sub_category_sanpham_ncc($conn, $main, $category)
	{
		$tach_category = explode(',', $category);
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if (in_array($r_tt['cat_id'], $tach_category) == true) {
				$list_id .= $r_tt['cat_id'] . ',';
				$list .= '<div class="li_input li_input_' . $r_tt['cat_main'] . '" id="input_' . $r_tt['cat_id'] . '"><input type="checkbox" name="category_ncc[]" main_id="' . $r_tt['cat_main'] . '" value="' . $r_tt['cat_id'] . '" checked> ' . $r_tt['cat_tieude'] . '</div>';
			} else {
				$list .= '<div class="li_input li_input_' . $r_tt['cat_main'] . '" id="input_' . $r_tt['cat_id'] . '"><input type="checkbox" name="category_ncc[]" main_id="' . $r_tt['cat_main'] . '" value="' . $r_tt['cat_id'] . '"> ' . $r_tt['cat_tieude'] . '</div>';
			}
		}
		$list_id = substr($list_id, 0, -1);
		return json_encode(array('list' => $list, 'list_id' => $list_id, 'total' => $total));
	}
	function list_div_sub_sub_category_sanpham_ncc($conn, $main, $category)
	{
		$tach_category = explode(',', $category);
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if (in_array($r_tt['cat_id'], $tach_category) == true) {
				$list_id .= $r_tt['cat_id'] . ',';
				$list .= '<div class="li_input li_input_' . $r_tt['cat_main'] . ' li_input_main_' . $r_tt['main_main'] . '" id="input_' . $r_tt['cat_id'] . '"><input type="checkbox" name="category_ncc[]" value="' . $r_tt['cat_id'] . '" checked> ' . $r_tt['cat_tieude'] . '</div>';
			} else {
				$list .= '<div class="li_input li_input_' . $r_tt['cat_main'] . ' li_input_main_' . $r_tt['main_main'] . '" id="input_' . $r_tt['cat_id'] . '"><input type="checkbox" name="category_ncc[]" value="' . $r_tt['cat_id'] . '"> ' . $r_tt['cat_tieude'] . '</div>';
			}
		}
		$list_id = substr($list_id, 0, -1);
		return json_encode(array('list' => $list, 'list_id' => $list_id, 'total' => $total));
	...
	function list_brand($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			$r_tt['i'] = $i;
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_brand', $r_tt);
		}
		return $list;
	}
	////////////////////
	function list_size($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			$r_tt['i'] = $i;
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_size', $r_tt);
		}
		return $list;
	}
	/////////////////
	function list_baiviet($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			$r_tt['i'] = $i;
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_baiviet', $r_tt);
		}
		return $list;
	}
	///////////////////
	function list_thongbao($conn, $user_id, $created, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$r_tt['date_post'] = date('d/m/Y', $r_tt['date_post']);
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_thongbao', $r_tt);
		}
		return $list;
	}
	///////////////////
	function list_remarketing($conn, $user_id, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$r_tt['date_post'] = date('d/m/Y', $r_tt['date_post']);
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_remarketing', $r_tt);
		}
		return $list;
	}
	///////////////////
	function list_thongbao_right($conn, $id, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$r_tt['date_post'] = date('d/m/Y', $r_tt['date_post']);
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_thongbao_right', $r_tt);
		}
		return $list;
	}
	///////////////////
	function list_lienhe($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$r_tt['date_post'] = date('d/m/Y', $r_tt['date_post']);
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_lienhe', $r_tt);
		}
		return $list;
	}
	///////////////////
	function list_nhantin($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$r_tt['i'] = $i;
			$r_tt['date_post'] = date('d/m/Y', $r_tt['date_post']);
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_nhantin', $r_tt);
		}
		return $list;
	}
	///////////////////
	function list_tichdiem($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$r_tt['i'] = $i;
			$r_tt['date_post'] = date('d/m/Y', $r_tt['date_post']);
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_tichdiem', $r_tt);
		}
		return $list;
	}
	///////////////////
	function list_thanhvien($conn, $shop, $active, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if ($active == 2) {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_thanhvien_khoa', $r_tt);
			} else if ($active == 3) {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_thanhvien_khoa_vinhvien', $r_tt);
			} else {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_thanhvien', $r_tt);
			}
		}
		return $list;
	}
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if ($r_tt['drop_min'] == 0) {
				if ($leader == 1 or $gia_leader == 1) {
					...
					$loi_nhuan = $r_tt['gia_moi'] - $r_tt['gia_ncc'];
				}
			} else {
				if ($leader == 1 or $gia_leader == 1) {
					$loi_nhuan = $r_tt['drop_min'] - $r_tt['gia_drop'];
				} else {
					$loi_nhuan = $r_tt['drop_min'] - $r_tt['gia_ncc'];
				}
			}
			...
			if ($leader == 1 or $gia_leader == 1) {
				...
				$r_tt['gia_nhap'] = number_format($r_tt['gia_ncc']);
			}
			if ($r_tt['drop_min'] == 0) {
			...
			if ($kieu == 'mobile') {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_sanpham_drop_mobile', $r_tt);
			} else {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_sanpham_drop', $r_tt);
			}
		}
		return $list;
	}
	...
	function list_link_affiliate($conn, $user_id, $kieu, $brand, $cat, $key, $sort, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if ($kieu == 'mobile') {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_link_aff_mobile', $r_tt);
			} else {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_link_aff', $r_tt);
			}
		}
		return $list;
	}
	...
	function list_sanpham($conn, $leader, $gia_leader, $kieu, $kho, $list_dang, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$r_tt['drop_max'] = number_format($r_tt['drop_max']);
			if ($kieu == 'mobile') {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_sanpham_mobile', $r_tt);
			} else {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_sanpham', $r_tt);
			}
		}
		return $list;
	}
	...
	function list_sanpham_trend($conn, $leader, $gia_leader, $kieu, $kho, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$r_tt['gia'] = number_format($r_tt['gia']);
			if ($kieu == 'mobile') {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_sanpham_trend_add_mobile', $r_tt);
			} else {
				$list .= $skin->skin_replace('skin_ncc/box_action/tr_sanpham_trend_add', $r_tt);
			}
		}
		return $list;
	}
	...
	function list_sanpham_tuan($conn, $leader, $gia_leader, $kho, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			if ($leader == 1 or $gia_leader == 1) {
				...
				if ($r_tt['gia_ncc_tuan'] == 0) {
					$gia_ncc = $r_tt['gia_tuan'] + (($r_tt['drop_min'] - $r_tt['gia_tuan']) * 0.3);
				} else {
					$gia_ncc = $r_tt['gia_ncc_tuan'];
				}
				if ($r_tt['gia_ncc_truoc'] == 0) {
					$gia = $r_tt['gia_truoc'] + (($r_tt['drop_min'] - $r_tt['gia_truoc']) * 0.3);
				} else {
					$gia = $r_tt['gia_ncc_truoc'];
				}
				$r_tt['gia_tuan'] = number_format($gia_ncc);
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_sanpham_tuan', $r_tt);
		}
		return $list;
	}
	///////////////////
	...
	function list_sanpham_hethang_catma($conn, $leader, $gia_leader, $kho, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_sanpham_hethang', $r_tt);
		}
		return $list;
	}
	///////////////////
	...
	function thongke_donhang_thanhvien_nam($conn, $user_id, $dau, $cuoi)
	{
		...
		$thongtin_ncc = mysqli_query($conn, "SELECT * FROM donhang_ncc WHERE date_post>='$dau' AND date_post<='$cuoi' AND user_id='$user_id'");
		while ($r_tt_ncc = mysqli_fetch_assoc($thongtin_ncc)) {
			$month = date('m', $r_tt_ncc['date_post']);
			$month = intval($month);
			if ($month == 1) {
	...
	////////////////////
	function list_kq_timkiem_donhang_ncc($conn, $user_id, $key)
	{
		$skin = $this->load('class_skin_cpanel');
		$check = $this->load('class_check');
		$i = 0;
		$thongtin = mysqli_query($conn, "SELECT * FROM donhang WHERE (ma_don LIKE '%$key%' OR ho_ten LIKE '%$key%' OR email LIKE '%$key%' OR dien_thoai LIKE '%$key%') AND ncc='1' AND user_id='$user_id' ORDER BY date_post DESC");
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			$r_tt['i'] = $i;
			$r_tt['tongtien'] = number_format($r_tt['tongtien']) . 'đ';
	...
	////////////////////
	function list_donhang_ncc($conn, $user_id, $search, $status, $date_from, $date_to, $page, $limit)
	{
		$skin = $this->load('class_skin_cpanel');
		$check = $this->load('class_check');
		$i = 0;
		...
		// Escape các biến đầu vào để tránh SQL Injection
		$search = mysqli_real_escape_string($conn, $search);
		$user_id = (int) $user_id;
		$status = (int) $status;
		...
		if (!empty($search)) {
			$where .= " AND (ma_don LIKE '%$search%' OR ho_ten LIKE '%$search%' OR email LIKE '%$search%' OR dien_thoai LIKE '%$search%')";
		}
		...
		if (!empty($date_from) && !empty($date_to)) {
			$date_from_unix = strtotime($date_from . " 00:00:00");
			$date_to_unix = strtotime($date_to . " 23:59:59");
			$where .= " AND date_post BETWEEN $date_from_unix AND $date_to_unix";
		}

		// Truy vấn chính
		$query = "SELECT * FROM donhang_shop WHERE $where ORDER BY date_post DESC LIMIT $offset, $limit";
		$result = mysqli_query($conn, $query);

		// // Xử lý kết quả
		while ($r_tt = mysqli_fetch_assoc($result)) {
			$i++;
			$r_tt['i'] = $i;
			$r_tt['tongtien'] = number_format($r_tt['tongtien']) . 'đ';
			$r_tt['date_post'] = date('d/m/Y', $r_tt['date_post']);
	...
	function list_donhang_socdo_tk($conn, $user_id, $search, $status, $date_from, $date_to, $page, $limit)
	{
		$skin = $this->load('class_skin_cpanel');
		$check = $this->load('class_check');
		$start = $page * $limit - $limit;
		...
		if (!empty($date_from) && !empty($date_to)) {
			...
			$where .= " AND (date_post BETWEEN $from_timestamp AND $to_timestamp OR date_update BETWEEN $from_timestamp AND $to_timestamp)";
		}

		$query = "SELECT * FROM donhang WHERE $where ORDER BY date_post DESC LIMIT $start,$limit";
		$thongtin = mysqli_query($conn, $query);
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list_sanpham = '';
			foreach ($tach_sanpham as $key => $value) {
				$size = !empty($value['size']) ? ' - Size: ' . strtoupper($value['size']) : '';
				$color = !empty($value['color']) ? ' - Màu: ' . $value['color'] : '';
				$list_sanpham .= '+' . $value['tieu_de'] . $color . $size . '<br>';
			}
			$r_tt['list_sanpham'] = $list_sanpham;
	...
	function list_kq_timkiem_donhang_socdo($conn, $user_id, $key)
	{
		...
		$check = $this->load('class_check');
		$i = 0;
		$thongtin = mysqli_query($conn, "SELECT * FROM donhang_ncc WHERE user_id='$user_id' AND (ma_don LIKE '%$key%' OR ho_ten LIKE '%$key%' OR email LIKE '%$key%' OR dien_thoai LIKE '%$key%') ORDER BY date_post DESC");
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			$r_tt['i'] = $i;
			$r_tt['tongtien'] = number_format($r_tt['tongtien']) . 'đ';
	...
	{
		$tlca_skin_ncc = $this->load('class_skin_cpanel');
		$check = $this->load('class_check');
		$start = $page * $limit - $limit;
		$i = $start;
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			...
			$list .= $tlca_skin_ncc->skin_replace('skin_ncc/box_action/tr_setting', $r_tt);
		}
		return $list;
	}
	///////////////////////
	...
	function list_banner($conn, $id)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($data)) {
			...
			$i++;
			$r_tt['i'] = $i;
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_banner', $r_tt);
		}
		return $list;
	}
	...
	function list_color($conn, $shop, $page, $limit)
	{
		...
		while ($r_tt = mysqli_fetch_assoc($thongtin)) {
			$i++;
			$r_tt['i'] = $i;
			$list .= $skin->skin_replace('skin_ncc/box_action/tr_color', $r_tt);
		}
		return $list;
	}
}
