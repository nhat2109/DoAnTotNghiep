...
if ($r_tt['total'] == 0) {
    ...
    // Calculate max old price and min new price from variants
    $gia_cu_max = 0;
    $gia_moi_min = PHP_INT_MAX;
    if (is_array($tach_phanloai) && !empty($tach_phanloai)) {
    ...
    // Get first variant info for other fields
    ...
    if ($ok == 1) {
        ...
        foreach ($tach_phanloai as $value) {
            ...
            if (in_array($ma_sp, $existing_variants)) {
                ...
                foreach ($variant_data as $key => $val) {
                    $set_clauses[] = is_string($val) ? "$key='$val'" : "$key=$val";
                }
        ...
        $thongbao = $thongbao ?? 'Lỗi khi cập nhật sản phẩm: ' . mysqli_error($conn);
    }
}
