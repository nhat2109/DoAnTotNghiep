<?php
	...
	while ($r_tt = mysqli_fetch_assoc($thongtin)) {
		...
		if ($kieu == 'tang') {
			$r_tt['gia_deal'] = 0;
			$r_tt['sale_deal'] = 100;
			$list .= $skin->skin_replace('skin_ncc/box_action/li_product_sub_tang', $r_tt);
		} else if ($kieu == 'muakem') {
			$r_tt['gia_deal'] = '';
			$r_tt['sale_deal'] = '';
			$list .= $skin->skin_replace('skin_ncc/box_action/li_product_sub_deal', $r_tt);
		} else {
			$r_tt['gia_deal'] = '';
			$r_tt['sale_deal'] = '';
			$list .= $skin->skin_replace('skin_ncc/box_action/li_product_flash_sale', $r_tt);
