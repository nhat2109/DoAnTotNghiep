	...
	if($total_follow==0){
		...
		if($list_id==''){
			...
			$bien = array(
				...
				'phantrang' => $class_index->phantrang($page, $total_page, '/dropship/list-sanpham-follow'),
			);
		}
	}
	if($kieu=='mobile'){
		$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/list_sanpham_follow_mobile', $bien);
	}else{
		$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/list_sanpham_follow', $bien);
	}
?>