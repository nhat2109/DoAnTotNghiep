<?php
	...
	if($list_id==''){
		...
		$bien = array(
			...
			'loai'=>$loai,
			'status'=>$status,
			'phantrang' => $class_index->phantrang_timkiem($page, $total_page, '/dropship/list-donhang-nhom?loai='.$loai.'&status='.$status),
		);
	}
	$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/list_donhang_nhom', $bien);
?>