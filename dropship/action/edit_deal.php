<?php
	...
	while ($r_sp = mysqli_fetch_assoc($thongtin_sanpham)) {
		...
		if (in_array($r_sp['id'], $tach_main) == true) {
			$list_main .= $skin->skin_replace('skin_dropship/box_action/li_product_main_deal', $r_sp);
		} else if (in_array($r_sp['id'], $tach_sub) == true) {
			$sp_id = $r_sp['id'];
			$r_sp['gia_deal'] = $tach_sp_sub[$sp_id]['gia'];
			$r_sp['sale_deal'] = $tach_sp_sub[$sp_id]['sale'];
			if ($r_tt['loai'] == 'tang') {
				$list_sub .= $skin->skin_replace('skin_dropship/box_action/li_product_sub_tang', $r_sp);
			} else {
				$list_sub .= $skin->skin_replace('skin_dropship/box_action/li_product_sub_deal', $r_sp);
			}
		}
	}
	$r_tt['list_main'] = $list_main;
	$r_tt['list_sub'] = $list_sub;
	$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/edit_deal', $r_tt);
?>