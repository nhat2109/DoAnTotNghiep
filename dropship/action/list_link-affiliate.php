<?php
	...
	if(strpos($link_hientai, '?')!==false AND strpos($link_hientai, '?page=')==false){
		...
		$sort='time-desc';
		$link_phantrang='/dropship/list-link-affiliate';
	...
	if($check->is_mobile()==true){
		...
		$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/list_link_affiliate_mobile', $r_tt);
	}else{
		$r_tt['list_sanpham'] = $class_index->list_link_affiliate($conn,$user_id,'laptop',$brand,$cat,$key,$sort, $page, $limit);
		$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/list_link_affiliate', $r_tt);
	}
?>