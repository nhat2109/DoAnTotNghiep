	...
	if ($step == 2) {
		...
		if($list_id==''){
			...
			$replace = array(
				...
				'thongbao' => $thongbao,
				'link_chuyen' => '/dropship/add-donhang-drop',
			);
		...
		while ($r_cart = mysqli_fetch_assoc($thongtin_cart)) {
					...
					if ($r_pl['size'] == $_SESSION['drop_cart'][$id_sp]['size']) {
						if(in_array($r_pl['size'], $list_id_size)==false){
							...
							if($user_info['leader']==1 OR $user_info['gia_leader']==1){
								...
								$list_size .= '<option value="'.$r_pl['size'].'"  gia="'.$r_pl['gia_ctv'].'" selected>'.$r_pl['ten_size'].'</option>';
							}
						}
						if($color==$r_pl['color']){
						...
						if(in_array($r_pl['size'], $list_id_size)==false){
							...
							if($user_info['leader']==1 OR $user_info['gia_leader']==1){
								...
								$list_size .= '<option value="'.$r_pl['size'].'" gia="'.$r_pl['gia_ctv'].'">'.$r_pl['ten_size'].'</option>';
							}
						}
					}
				}
			...
			$list_shopcart .= $skin->skin_replace('skin_dropship/box_action/li_shopcart', $r_cart);
			unset($list_id_color);
			unset($list_id_size);
		...
		$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/add_donhang_drop_step2', $r_tt);
	} else if ($step == 3) {
		$type = $url_query['type'];
		if($type=='gopdon'){
			...
			if($type=='gopdon'){
				for ($i=0; $i < $_SESSION['drop_cart'][$id_sp]['quantity']; $i++) { 
					...
					if($user_info['leader']==1 OR $user_info['gia_leader']==1){
						...
						//$total_price += $r_cart['gia_ctv'] * 1;
						$total_gia_ban += $r_cart['gia_moi'] * 1;
						$thanhtien = $product_pl[$id_sp]['gia_ctv'] * 1;
						$thanhtien_gop=$thanhtien - ($thanhtien/100)*$giam_g;
						$total_price += $thanhtien - ($thanhtien/100)*$giam_g;
						$r_cart['thanhtien']=number_format($thanhtien);
						$r_cart['thanhtien_gop']=number_format($thanhtien_gop);
					...
					$r_cart['quantity'] = $_SESSION['drop_cart'][$id_sp]['quantity'];
					if($sp_g<3){
						$list_shopcart .= $skin->skin_replace('skin_dropship/box_action/li_shopcart_gopdon_no_right', $r_cart);
					}else if($sp_g<=5){
						$list_shopcart .= $skin->skin_replace('skin_dropship/box_action/li_shopcart_gopdon_right', $r_cart);
					}else{
						$list_shopcart .= $skin->skin_replace('skin_dropship/box_action/li_shopcart_gopdon_no_right', $r_cart);
					}
				...
				$r_cart['quantity'] = $_SESSION['drop_cart'][$id_sp]['quantity'];
				$list_shopcart .= $skin->skin_replace('skin_dropship/box_action/li_shopcart_right', $r_cart);
			}
		}
		$phi_ship = 0;
		...
		if($type=='san'){
			$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/add_donhang_drop_step3', $r_tt);
		}else if($type=='gopdon'){
			$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/add_donhang_gopdon_step3', $r_tt);
		}else{
			$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/add_donhang_ctv_step3', $r_tt);
		}
	} else {
		...
		if(strpos($link_hientai, '?')!==false AND strpos($link_hientai, '?page=')==false){
			...
			$sort='time-desc';
			$link_phantrang='/dropship/add-donhang-drop';
		...
		if($check->is_mobile()==true){
			...
			$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/add_donhang_drop_step1_mobile', $r_tt);
		}else{
			...
			$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/add_donhang_drop_step1', $r_tt);
		}
	}
?>