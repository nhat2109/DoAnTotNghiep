...
while ($pl = mysqli_fetch_assoc($thongtin_phanloai)) {
    ...
    $list_phanloai .= $skin->skin_replace('skin_dropship/box_action/li_phanloai', $pl);
}
...
if (strlen($r_tt['thongtin']) > 2) {
	...
	foreach ($tach_info as $key => $value) {
		$tach_value = explode('&&', $value);
		$list_info .= $skin->skin_replace('skin_dropship/box_action/li_info', $tach_value);
	}
	$r_tt['list_info'] = $list_info;
} else {
	$r_tt['list_info'] = '';
...
if ($r_tt['ma_sanpham'] != '') {
	if (strpos($r_tt['ma_sanpham'], '|') !== false) {
		...
		foreach ($tach_ma_sanpham as $key => $value) {
			$tach_value = explode('&&', $value);
			$list_ma .= $skin->skin_replace('skin_dropship/box_action/li_ma_sanpham', $tach_value);
		}
	} else {
		$tach_ma_sanpham = explode('&&', $r_tt['ma_sanpham']);
		$list_ma .= $skin->skin_replace('skin_dropship/box_action/li_ma_sanpham', $tach_ma_sanpham);
	}
	...
	if ($r_tt['sp_id'] > 0) {
		...
		$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/edit_sanpham', $r_tt);
	} else if ($r_tt['link_aff'] != '') {
		$r_tt['option_size'] = $class_index->list_option_size($conn, $user_id, $r_tt['size']);
		$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/edit_sanpham_affiliate', $r_tt);
	} else {
		$r_tt['option_size'] = $class_index->list_option_size($conn, $user_id, $r_tt['size']);
		$thaythe['box_right'] = $skin->skin_replace('skin_dropship/box_action/edit_sanpham_ngoai', $r_tt);
	}
?>