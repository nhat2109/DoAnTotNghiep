<?php
	...
	$kho = preg_replace('/[^0-9]/', '', $_REQUEST['kho']);
	...
	if ($r_tt['total'] == 0) {
		...
		if (in_array($duoi, array('jpg', 'jpeg', 'png', 'gif', 'webp')) == true) {
			...
			if ($link == $link_old) {
				...
				mysqli_query($conn, "UPDATE sanpham_shop SET tieu_de='$tieu_de',kho_hang='$kho',cat='$category',gia_cu='$gia_cu',gia_moi='$gia_moi',noi_bat='$noibat',noi_dung='$noidung',mau='$color',thuong_hieu='$thuong_hieu',thongtin='$info',can_nang='$can_nang',size='$size',minh_hoa='$minh_hoa',anh='$anh',title='$title',description='$description' WHERE id='$id' AND shop='$user_id'");
				...
				if ($r_seo['total'] == 0) {
					...
					mysqli_query($conn, "UPDATE sanpham_shop SET tieu_de='$tieu_de',kho_hang='$kho',cat='$category',link='$link',gia_cu='$gia_cu',gia_moi='$gia_moi',noi_bat='$noibat',noi_dung='$noidung',mau='$color',thuong_hieu='$thuong_hieu',thongtin='$info',can_nang='$can_nang',size='$size',minh_hoa='$minh_hoa',anh='$anh',title='$title',description='$description' WHERE id='$id' AND shop='$user_id'");
			...
			if ($link == $link_old) {
				...
				mysqli_query($conn, "UPDATE sanpham_shop SET tieu_de='$tieu_de',kho_hang='$kho',cat='$category',gia_cu='$gia_cu',gia_moi='$gia_moi',noi_bat='$noibat',noi_dung='$noidung',mau='$color',thuong_hieu='$thuong_hieu',thongtin='$info',can_nang='$can_nang',size='$size',anh='$anh',title='$title',description='$description' WHERE id='$id' AND shop='$user_id'");
				$thongbao = 'Sửa sản phẩm thành công';
				$ok = 1;
			} else {
				$thongtin_seo = mysqli_query($conn, "SELECT *, count(*) AS total FROM seo_shop WHERE link='$link' AND loai='sanpham' AND shop='$user_id'");
				...
				if ($r_seo['total'] == 0) {
					...
					mysqli_query($conn, "UPDATE sanpham_shop SET tieu_de='$tieu_de',kho_hang='$kho',cat='$category',link='$link',gia_cu='$gia_cu',gia_moi='$gia_moi',noi_bat='$noibat',noi_dung='$noidung',mau='$color',thuong_hieu='$thuong_hieu',thongtin='$info',can_nang='$can_nang',size='$size',anh='$anh',title='$title',description='$description' WHERE id='$id' AND shop='$user_id'");
					mysqli_query($conn, "UPDATE seo_shop SET link='$link' WHERE link='$link_old' AND loai='sanpham' AND shop='$user_id'");
					$thongbao = 'Sửa sản phẩm thành công';
					$ok = 1;
				} else {
