<?php
	...
	$tamtinh=preg_replace('/[^0-9]/', '', $_REQUEST['tamtinh']);
	$don=intval($_REQUEST['don']);
	$tinh_gui=1;
	$huyen_gui=25;
	$list_id='';
	if(isset($_SESSION['don'][$don])){
	...
	if($list_id==''){
		$ok=0;
		echo json_encode(array('ok'=>$ok,'list' => ''));
	}else{
		...
		while ($r_cart = mysqli_fetch_assoc($thongtin_cart)) {
			$id_sp = $r_cart['id'];
			for ($i=1; $i <= $so_luong[$id_sp]; $i++) { 
				$total_banle += $product_pl[$id_sp]['gia_moi'];
				$can_nang+=str_replace(',', '.', $product_pl[$id_sp]['can_nang']);
			}
		}
		...
		if($congty_ship=='ninja_van'){
			...
			$list=$class_viettel->option_dichvu($token_client,$trongluong,$tamtinh,$tien_thu,$huyen_gui,$tinh_gui,$huyen_nhan,$tinh_nhan);
		}
		echo json_encode(array('ok'=>$ok,'list' => $list));
	}
?>