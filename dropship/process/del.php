<?php
			...
			if ($loai == 'category') {
				...
				if ($r_tt['total'] == 0) {
					...
					mysqli_query($conn, "DELETE FROM bom_hang WHERE id='$id' AND shop='$user_id'");
				}
				...
				if (!$thongtin) {
					echo json_encode(array(
						'ok' => 0,
						'thongbao' => 'Lỗi truy vấn database'
					));
					exit;
				}
				...
				if ($r_tt['total'] == 0) {
					$ok = 0;
					$thongbao = 'Màu sắc không tồn tại';
				} else {
					$result = mysqli_query($conn, "DELETE FROM mau_sanpham WHERE id='$id' AND shop='$user_id'");
					if (!$result) {
						echo json_encode(array(
							'ok' => 0,
							'thongbao' => 'Lỗi khi xóa dữ liệu'
						));
						exit;
					}
					$ok = 1;
					$thongbao = 'Xóa màu sắc thành công';
				}
			}
			$info = array(
				'ok' => $ok,
