...
if (!isset($_COOKIE['user_id'])) {
	...
	// anh hieu
	 // Lưu user_id vào session để sử dụng trong các file khác
	 ...
	 // Lấy danh sách sản phẩm (cho popup)
		 ...
		 $result = mysqli_query($conn, "SELECT sp_id, tieu_de, gia_cu, gia_moi, minh_hoa FROM sanpham_shop WHERE $where");
        ...
        while ($variant = mysqli_fetch_assoc($variant_result)) {
            $variants[] = [
                ...
                'stock' => intval($variant['kho_sanpham_shop']) // Thêm trường stock từ kho_sanpham_shop
            ];
        }
	...
	//Huyphuc
		...
		if (file_exists($file_path)) {
			unlink($file_path); // Xóa file khỏi thư mục
			$ok = 1;
			$thongbao = 'Xóa ảnh thành công!';
		} else {
			$ok = 0;
			$thongbao = 'Ảnh không tồn tại!';
		}
