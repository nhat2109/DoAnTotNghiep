...
if(in_array($web, $web_root)==false){
	include('./shop/view_chungnhan.php');
	exit();
}
...
$thongtin=mysqli_query($conn,"SELECT *,count(*) AS total FROM post WHERE link='$blank' ORDER BY id DESC LIMIT 1");
...
mysqli_query($conn,"UPDATE post SET view='$view_new' WHERE id='{$r_tt['id']}'");
...
$noidung=preg_replace('/<p>&nbsp;<\/p>/', '', $r_tt['noidung']);
$replace=array(
	...
	'noi_dung'=>$check->remove_blank_line($r_tt['noidung']),
...
echo $skin->skin_replace('skin/view_chungnhan',$replace);
?>