...
if(in_array($user_id, $tach_doc)==true){
	...
	if($r_tt['doc']==''){
		mysqli_query($conn,"UPDATE thongbao_shop SET doc='$user_id' WHERE id='$id' AND shop='$shop'");
	}else{
		$doc=$r_tt['doc'].','.$user_id;
		mysqli_query($conn,"UPDATE thongbao_shop SET doc='$doc' WHERE id='$id' AND shop='$shop'");
	}
}
if($r_tt['status']==1){
...
echo $skin->skin_replace('skin_shop/'.$s.'/tpl/thongbao_detail',$replace);
?>