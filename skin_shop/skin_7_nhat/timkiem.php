...
if(strlen($where_tukhoa)<5){
	$thongke=mysqli_query($conn,"SELECT count(*) AS total FROM sanpham_shop WHERE shop='{$r_shop['user_id']}'");
}else{
	$thongke=mysqli_query($conn,"SELECT count(*) AS total FROM sanpham_shop WHERE ".$where_tukhoa." AND shop='{$r_shop['user_id']}'");
}
...
$replace = array(
	...
	'list_sanpham'=>$class_index->list_sanpham_timkiem($conn,$s,$r_shop['user_id'],$list_muakem_id,$list_tang_id,$list_flashsale_id,$list_c,$where_tukhoa,$order,$page,$limit),
	'total_sanpham'=>$r_tk['total'],
	...
	'noidung_header' => $_SESSION['noidung_header'], // Ghi đè giá trị của noidung_headerss
	);
...
echo $skin->skin_replace('skin_shop/'.$s.'/tpl/timkiem',$replace);
?>