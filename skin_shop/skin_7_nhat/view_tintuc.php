...
//  var_dump($_REQUEST['blank']); exit();

$thongtin = mysqli_query($conn, "SELECT *,count(*) AS total FROM post_shop WHERE link='$blank' AND shop='{$r_shop['user_id']}'");
...
mysqli_query($conn, "UPDATE post_shop SET view='$view_new' WHERE id='{$r_tt['id']}' AND shop='{$r_tt['shop']}'");
...
$replace = array(
	...
	'list_category_sub'=>$list_category_sub,
	'noi_dung' => $r_tt['noidung'],
	'date_post' => date('d/m/Y', $r_tt['date_post']),
	'list_baiviet_moi' => $class_index->list_tintuc_moi($conn, $s, $r_tt['shop'], 5),
	'list_lienquan' => $class_index->list_baiviet_lienquan($conn, $s, $r_tt['shop'], $r_tt['id'], $r_tt['cat'], 4),
...
echo $skin->skin_replace('skin_shop/' . $s . '/tpl/view_tintuc', $replace);
?>