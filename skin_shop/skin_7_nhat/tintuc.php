...
$thongtin=mysqli_query($conn,"SELECT *,count(*) AS total FROM category_shop WHERE cat_blank='$blank' AND shop='{$r_shop['user_id']}' ORDER BY cat_id DESC LIMIT 1");
...
$cat_id=$r_tt['cat_id'];
$thongke=mysqli_query($conn,"SELECT count(*) AS total FROM post_shop WHERE FIND_IN_SET($cat_id,cat)>0");
$r_tk=mysqli_fetch_assoc($thongke);
$total_page=ceil($r_tk['total']/$limit);
$phantrang=$class_index->phantrang($page,$total_page,'/bai-viet/'.$blank.'.html');
$tach_list=json_decode($class_index->list_baiviet_category($conn,$s,$r_shop['user_id'],$cat_id,$page,$limit),true);
...
$replace = array(
	...
	'list_baiviet_moi'=>$class_index->list_tintuc_moi($conn,$s,$r_tt['shop'],5),
...
echo $skin->skin_replace('skin_shop/'.$s.'/tpl/tintuc',$replace);
?>