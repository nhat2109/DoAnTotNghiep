...
function check_post(id) {
    $.ajax({
        ...
        success: function (kq) {
            var info = JSON.parse(kq);
            if (info.ok == 1) {
                window.location.href = '/dropship/add-sanpham?step=2&id=' + id
            } else {
                $('.load_overlay').show();
                $('.load_process').fadeIn();
...
$(document).ready(function () {
    ...
    $('#sort_link_affiliate').on('change', function () {
        sort = $(this).val();
        window.location.href = '/dropship/add-donhang-drop?sort=' + sort;
    });
    ...
    if (!$('.toast-container').length) {
      ...
      $('body').on('click', '.add_follow', function () {
        ...
        // Dùng class "added" để kiểm tra trạng thái đã follow hay chưa
        var isAdded = $btn.hasClass('added');
        var loai = isAdded ? 'remove' : 'add';
      
        // Cập nhật trạng thái nút ngay lập tức (thay đổi border thông qua CSS)
        if (!isAdded) {
            $btn.addClass('added');
        } else {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // Tùy thuộc vào trạng thái, tạo thông báo với nội dung và kiểu CSS khác nhau
                var message, toastClass;
                if (loai === 'add') {
                    message = 'Đã thêm vào yêu thích <i class="fa fa-check"></i>';
                    toastClass = 'toast-add';
                } else {
                    message = 'Đã xóa khỏi yêu thích <i class="fa fa-times"></i>';
                    toastClass = 'toast-remove';
                }     
                // Tạo phần tử thông báo mới
                var $notification = $('<div class="toast-notification ' + toastClass + '"></div>');
                $notification.html(message);
                
                // Thêm thông báo mới vào container thông báo (prepend để thông báo mới ở trên cùng)
                $('.toast-container').prepend($notification);
                
                // Kích hoạt animation thông qua class "show"
                setTimeout(function(){
                    $notification.addClass('show');
                }, 50);
                
                // Sau 2 giây, ẩn thông báo và xoá khỏi DOM sau animation
                setTimeout(function () {
        ...
        $.ajax({
            ...
            success: function (kq) {
                ...
                setTimeout(function () {
                    ...
                    $('.load_overlay').hide();
                    if (info.ok == 1) {
                        window.location.href = '/dropship/list-giaodien?step=2';
                    }
                }, 3000);
            }
        });
        ...
        $.ajax({
            ...
            success: function (kq) {
                ...
                setTimeout(function () {
                    ...
                    $('.load_overlay').hide();
                    if (info.ok == 1) {
                        window.location.href = '/dropship/add-naptien?step=2';
                    }
                }, 3000);
            }
        });
                ...
                success: function (kq) {
                    ...
                    setTimeout(function () {
                        ...
                        if (info.ok == 1) {
                            ...
                            window.location.href = '/dropship/list-donhang-dropship';
                        } else {
    ...
    /////////////////////////////
    $('button[name=button_hoanthanh_ctv]').on('click', function () {
        ho_ten = $('input[name=ho_ten]').val();
        dien_thoai = $('input[name=dien_thoai]').val();
        dia_chi = $('input[name=dia_chi]').val();
        ghi_chu = $('textarea[name=ghi_chu]').val();
        ...
        if (cod == '') {
            ...
            var form_data = new FormData();
            form_data.append('action', 'hoanthanh_donhang_ctv');
            form_data.append('ho_ten', ho_ten);
            form_data.append('dien_thoai', dien_thoai);
            form_data.append('dia_chi', dia_chi);
            form_data.append('ghi_chu', ghi_chu);
                ...
                success: function (kq) {
                    ...
                    setTimeout(function () {
                        ...
                        if (info.ok == 1) {
                            ...
                            window.location.href = '/dropship/list-donhang-socdo';
                        } else {
                ...
                success: function (kq) {
                    ...
                    if (info.ok == 1) {
                        ...
                        setTimeout(function () {
                            ...
                            $('.load_note').html('Hệ thống đang xử lý');
                            $('.load_overlay').hide();
                            window.location.href = '/dropship/add-donhang-drop?step=2';
                        }, 3000);
                    } else {
                        setTimeout(function () {
                            $('.load_note').html(info.thongbao);
    ...
    $('input[name=input_search_donhang_dropship]').keypress(function (e) {
        key = $(this).val();
        if (e.which == 13) {
            if (key.length < 1) {
                $('input[name=input_search_donhang_dropship]').focus();
            } else {
                $('.load_overlay').show();
                $('.load_process').fadeIn();
                $.ajax({
                    ...
                    type: 'post',
                    data: {
                        action: 'timkiem_donhang_dropship',
                        key: key
                    },
                    success: function (kq) {
                        var info = JSON.parse(kq);
...
$('.button_timkiem_hieu').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (kq) {
                    ...
                    setTimeout(function () {
                        ...
                        $('.load_overlay').hide();
                        if (info.ok == 1) {
                            window.location.href = '/dropship/list-size';
                        } else {
            ...
            $.ajax({
                ...
                success: function (kq) {
                    ...
                    setTimeout(function () {
                        ...
                        $('.load_overlay').hide();
                        if (info.ok == 1) {
                            window.location.href = '/dropship/list-color';
                        } else {
            ...
            $.ajax({
                ...
                success: function (kq) {
                    ...
                    setTimeout(function () {
                        ...
                        $('.load_overlay').hide();
                        if (info.ok == 1) {
                            window.location.href = '/dropship/list-brand';
                        } else {
    ...
    $('.show_add_marketing').on('click', function () {
        window.location.href = '/dropship/add-remarketing';
    });
    //////////////////
        ...
        $.ajax({
            ...
            success: function (kq) {
                ...
                setTimeout(function () {
                    ...
                    $('.load_overlay').hide();
                    if (info.ok == 1) {
                        window.location.href = '/dropship/list-setting';
                    } else {
            ...
            $.ajax({
                ...
                success: function (kq) {
                    ...
                    setTimeout(function () {
                        ...
                        $('.load_overlay').hide();
                        if (info.ok == 1) {
                            window.location.href = '/dropship/list-menu';
                        }
                    }, 3000);
                }
            });
            ...
            $.ajax({
                ...
                success: function (kq) {
                    ...
                    setTimeout(function () {
                        ...
                        $('.load_overlay').hide();
                        if (info.ok == 1) {
                            window.location.href = '/dropship/list-slide';
                        } else {
            ...
            $.ajax({
                ...
                success: function (kq) {
                    ...
                    setTimeout(function () {
                        ...
                        $('.load_overlay').hide();
                        if (info.ok == 1) {
                            window.location.href = '/dropship/list-category';
                        } else {
        ...
        $.ajax({
            ...
            success: function (kq) {
                ...
                setTimeout(function () {
                    ...
                    $('.load_overlay').hide();
                    if (info.ok == 1) {
                        window.location.href = '/dropship/list-tuyendung-nhom';
                    } else {
            ...
            $.ajax({
                ...
                success: function (kq) {
                    ...
                    setTimeout(function () {
                        ...
                        $('.load_overlay').hide();
                        if (info.ok == 1) {
                            window.location.href = '/dropship/list-theloai';
                        } else {
   ...
   $('button[name=button_hoahong]').on('click', function () {
    ...
    $('button[name=login]').on('click', function () {
        ...
        if (username.length < 4) {
            ...
            $('.load_process').fadeIn();
            $.ajax({
                url: "/dropship/process_login.php",
                type: "post",
                data: {
                    username: username,
                    password: password,
                ...
                success: function (kq) {
                    ...
                    setTimeout(function () {
                        ...
                        $('.load_overlay').hide();
                        if (info.ok == 1) {
                            window.location.href = '/dropship/dashboard';
                        } else {
        ...
        $.ajax({
            ...
            success: function (kq) {
                ...
                setTimeout(function () {
                    ...
                    $('.load_overlay').hide();
                    if (info.ok == 1) {
                        window.location.href = '/dropship/add-donhang-drop?step=3&type=san';
                    }
                }, 3000);
        ...
        $.ajax({
            ...
            success: function (kq) {
                ...
                setTimeout(function () {
                    ...
                    $('.load_overlay').hide();
                    if (info.ok == 1) {
                        window.location.href = '/dropship/add-donhang-drop?step=3&type=gopdon';
                    }
                }, 3000);
        ...
        $.ajax({
            ...
            success: function (kq) {
                ...
                setTimeout(function () {
                    ...
                    $('.load_overlay').hide();
                    if (info.ok == 1) {
                        window.location.href = '/dropship/add-donhang-drop?step=3&type=socdo';
                    }
                }, 3000);
       ...
       $('.button_add_phanloai').on('click', function () {
        ...
        $('.list_phanloai').append(`
            <div class="li_phanloai">
                <div class="info_ma"><input type="text" name="ma[]" placeholder="Mã" value="${randomCode}"></div>
                <div class="info_name"><input type="text" name="size[]" giatri="" placeholder="Kích cỡ"><div class="list_goiy scroll"></div></div>
                <div class="info_mau"><input type="text" name="color[]" giatri="" placeholder="Màu sắc"><div class="list_goiy scroll"></div></div>
                <div class="info_can_nang"><input type="text" name="can_nang[]" placeholder="Trọng lượng (kg)"></div>
                <div class="info_gia"><input type="text" name="gia_cu[]" class="price_format" placeholder="Giá niêm yết"></div>
                <div class="info_gia"><input type="text" name="gia_moi[]" class="price_format" placeholder="Giá bán"></div>
    ...
    $('body').on('click', '.list_phanloai .list_goiy .li_goiy', function () {
        ...
        $input.val(text).attr('giatri', value);
        if ($(this).attr('ma_mau')) $input.attr('ma_mau', $(this).attr('ma_mau'));
        $(this).parent().html('');
    });
    ////////////////
    ...
    $('body').on('click', '.list_phanloai .li_phanloai .info_action', function () {
        $(this).closest('.li_phanloai').remove();
    });

    // Sao chép phân loại
    $('body').on('click', '.list_phanloai .li_phanloai .info_action_copy', function () {
        const $parent = $(this).closest('.li_phanloai');
        const randomCode = generateRandomCode();
        const newRow = $parent.clone();
        newRow.find('input[name="ma[]"]').val(randomCode);
        newRow.find('.list_goiy').html('');
        $('.list_phanloai').append(newRow);

    });
    // Tính trọng lượng tính ship dựa trên kích thước hộp
    $('body').on('input', '.list_phanloai input[name^="can_nang"], input[name="chieudai_shop"], input[name="chieurong_shop"], input[name="chieucao_shop"]', function () {
        const chieudai = parseFloat($('input[name="chieudai_shop"]').val()) || 0;
        const chieurong = parseFloat($('input[name="chieurong_shop"]').val()) || 0;
        const chieucao = parseFloat($('input[name="chieucao_shop"]').val()) || 0;
        const thetich = chieudai * chieurong * chieucao;
        const trongluongKichthuoc = thetich / 6000;

        $('.list_phanloai .li_phanloai').each(function () {
            const canNang = parseFloat($(this).find('input[name^="can_nang"]').val()) || 0;
            const canNangTinhShip = Math.max(canNang, trongluongKichthuoc);
            $(this).find('input[name^="trongluongtinhship"]').val(canNangTinhShip.toFixed(2));
        });
    });
    ...
    //     const chieudai_shop = $('input[name=chieudai_shop]').val();
    //     const chieurong_shop = $('input[name=chieurong_shop]').val();
    //     const chieucao_shop = $('input[name=chieucao_shop]').val();
    //     const list_photo = [];
    //     $('.list_photo img').each(function () {
    //         list_photo.push($(this).attr('src'));
    //     });
    //     const anh = list_photo.toString();
    //     const thuong_hieu = $('select[name=thuong_hieu]').val();
    //     const thuong_hieu_2 = $('input[name=thuong_hieu_2]').val();
    //     const list_info = $('.li_info').map(function () {
    //         const info_name = $(this).find('input[name^=info_name]').val();
    //         const info_value = $(this).find('input[name^=info_value]').val();
    //         return info_name ? `${info_name}&&${info_value}|` : '';
    //     }).get().join('');
    //     const list_cat = $('.li_input input[name^=category]:checked').map(function () {
    //         return $(this).val();
    //     }).get().join(',');
    //     const noibat = tinyMCE.get('noibat').getContent();
    //     const noidung = tinyMCE.get('edit_textarea').getContent();
    ...
    //     let pl = 0;
    //     $('.list_phanloai .li_phanloai').each(function () {
    //         const ma_sp = $(this).find('input[name^=ma]').val();
    //         const size = $(this).find('input[name^=size]').attr('giatri') || '';
    //         const color = $(this).find('input[name^=color]').attr('giatri') || '';
    //         const ma_mau = $(this).find('input[name^=color]').attr('ma_mau') || '';
    //         const ten_size = $(this).find('input[name^=size]').val();
    //         const ten_color = $(this).find('input[name^=color]').val();
    //         const can_nang = $(this).find('input[name^=can_nang]').val();
    //         const gia_cu = $(this).find('input[name^=gia_cu]').val();
    //         const gia_moi = $(this).find('input[name^=gia_moi]').val();
    //         const kho_sanpham_shop = $(this).find('input[name^=kho_sanpham_shop]').val();
    //         pl++;
    //         const jsonStr = `{"ma_sp":"${ma_sp}","color":"${color}","size":"${size}","ten_color":"${ten_color}","ma_mau":"${ma_mau}","ten_size":"${ten_size}","can_nang":"${can_nang}","gia_cu":"${gia_cu}","gia_moi":"${gia_moi}","kho_sanpham_shop":"${kho_sanpham_shop}"}`;
    //         list_phanloai += pl === 1 ? jsonStr : `,${jsonStr}`;
    //     });

    //     if (tieu_de.length < 4) {
    //         $('input[name=tieu_de]').focus();
    //     } else if (!document.getElementById("minh_hoa").files.length && !$('#preview-minhhoa').attr('src')) {
    //         $('.load_overlay').show();
    //         $('.load_process').fadeIn();
    //         setTimeout(() => $('.load_note').html('Vui lòng chọn hình minh họa'), 500);
    //         setTimeout(() => {
    //             $('.load_process').hide();
    //             $('.load_note').html('Hệ thống đang xử lý');
    //             $('.load_overlay').hide();
    //             $('html, body').animate({ scrollTop: $('#preview-minhhoa').offset().top - 150 }, 500);
    //         }, 2000);
    //     } else if (list_phanloai === '') {
    //         $('.load_overlay').show();
    //         $('.load_process').fadeIn();
    //         setTimeout(() => $('.load_note').html('Vui lòng thêm phân loại sản phẩm'), 500);
    //         setTimeout(() => {
    //             $('.load_process').hide();
    //             $('.load_note').html('Hệ thống đang xử lý');
    //             $('.load_overlay').hide();
    ...
    //         $('textarea[name=description]').focus();
    //     } else {
    //         const file_data = $('#minh_hoa').prop('files')[0];
    ...
    $("button[name=add_sanpham_ngoai]").on("click", function () {
        ...
        function stripHtml(html) {
            ...
            text = text
                .replace(/[\n\r]+/g, " ") // Thay thế dòng mới bằng khoảng trắng
        ...
        const showErrorMessage = (message, element) => {
            ...
            element.after(
                '<span class="error-message" style="color: red; font-size: 12px; display: block;">' +
                message +
                "</span>"
            );
        ...
        // Lấy dữ liệu từ form
        const tieu_de = $("input[name=tieu_de]").val().trim();
        const link = $("input[name=link]").val().trim();
        const chieudai_shop = $("input[name=chieudai_shop]").val().trim().replace(/,/g, '');
        const chieurong_shop = $("input[name=chieurong_shop]").val().trim().replace(/,/g, '');
        const chieucao_shop = $("input[name=chieucao_shop]").val().trim().replace(/,/g, '');
        const list_photo = [];
        $(".list_photo img").each(function () {
            list_photo.push($(this).attr("src"));
        });
        const anh = list_photo.toString();
        const thuong_hieu = $("select[name=thuong_hieu]").val();
        const thuong_hieu_2 = $("input[name=thuong_hieu_2]").val().trim();
        const list_info = $(".li_info")
            .map(function () {
                const info_name = $(this).find("input[name^=info_name]").val().trim();
                const info_value = $(this).find("input[name^=info_value]").val().trim();
                return info_name ? `${info_name}&&${info_value}|` : "";
            })
            .get()
            .join("");
        const list_cat = $(".li_input input[name^=category]:checked")
            .map(function () {
                return $(this).val();
            })
            .get()
            .join(",");
        const noibatRaw = tinyMCE.get("noibat").getContent().trim();
        const noibatText = stripHtml(noibatRaw); // Chỉ lấy văn bản thuần túy cho "Đặc điểm nổi bật"
        const noidungRaw = tinyMCE.get("edit_textarea").getContent().trim(); // Giữ nguyên HTML cho "Mô tả chi tiết"
        ...
        // Validate phân loại sản phẩm
        $(".list_phanloai .li_phanloai").each(function () {
            const ma_sp = $(this).find("input[name^=ma]").val().trim();
            const size = $(this).find("input[name^=size]").attr("giatri") || "";
            const color = $(this).find("input[name^=color]").attr("giatri") || "";
            const ma_mau = $(this).find("input[name^=color]").attr("ma_mau") || "";
            const ten_size = $(this).find("input[name^=size]").val().trim();
            const ten_color = $(this).find("input[name^=color]").val().trim();
            const can_nang = $(this).find("input[name^=can_nang]").val().trim().replace(/,/g, '');
            const gia_cu = $(this).find("input[name^=gia_cu]").val().trim().replace(/,/g, '');
            const gia_moi = $(this).find("input[name^=gia_moi]").val().trim().replace(/,/g, '');
            const kho_sanpham_shop = $(this).find("input[name^=kho_sanpham_shop]").val().trim().replace(/,/g, '');
    
            if (!size) {
                $(this)
                    .find(".info_name")
                    .append(
                        '<span class="error-message" style="color: red; font-size: 12px; display: block;">Nhập kích cỡ</span>'
                    );
                if (!hasError) {
                    $("html, body").animate(
                        { scrollTop: $(this).find(".info_name").offset().top - 100 },
                        500
                    );
                    hasError = true;
                }
            ...
            if (!color) {
                $(this)
                    .find(".info_mau")
                    .append(
                        '<span class="error-message" style="color: red; font-size: 12px; display: block;">Nhập màu sắc</span>'
                    );
                if (!hasError) {
                    $("html, body").animate(
                        { scrollTop: $(this).find(".info_mau").offset().top - 100 },
                        500
                    );
                    hasError = true;
                }
            ...
            if (!gia_moi) {
                $(this)
                    ...
                    .after(
                        '<span class="error-message" style="color: red; font-size: 12px; display: block;">Nhập giá bán lẻ.</span>'
                    );
                if (!hasError) {
                    $("html, body").animate(
                        {
                            scrollTop: $(this).find("input[name^=gia_moi]").offset().top - 100,
                        },
                        500
                    );
                    hasError = true;
                }
            } else if (isNaN(gia_moi) || Number(gia_moi) <= 0) {
                $(this)
                    .find("input[name^=gia_moi]")
                    .after(
                        '<span class="error-message" style="color: red; font-size: 12px; display: block;">Giá bán lẻ phải là số lớn hơn 0.</span>'
                    );
                if (!hasError) {
                    $("html, body").animate(
                        {
            ...
            if (gia_cu && (isNaN(gia_cu) || Number(gia_cu) < 0)) {
                $(this)
                    .find("input[name^=gia_cu]")
                    .after(
                        '<span class="error-message" style="color: red; font-size: 12px; display: block;">Giá niêm yết phải là số không âm.</span>'
                    );
                if (!hasError) {
                    $("html, body").animate(
                        {
                            scrollTop: $(this).find("input[name^=gia_cu]").offset().top - 100,
                        },
                        500
                    );
                    hasError = true;
            ...
            if (can_nang && (isNaN(can_nang) || Number(can_nang) <= 0)) {
                $(this)
                    .find("input[name^=can_nang]")
                    .after(
                        '<span class="error-message" style="color: red; font-size: 12px; display: block;">Cân nặng lớn hơn 0.</span>'
                    );
                if (!hasError) {
                    $("html, body").animate(
                        {
                            scrollTop: $(this).find("input[name^=can_nang]").offset().top - 100,
                        },
                        500
                    );
                    hasError = true;
            ...
            if (
                kho_sanpham_shop &&
                (isNaN(kho_sanpham_shop) || Number(kho_sanpham_shop) < 0)
            ) {
                $(this)
                    .find("input[name^=kho_sanpham_shop]")
                    .after(
                        '<span class="error-message" style="color: red; font-size: 12px; display: block;">Số lượng kho phải là số không âm.</span>'
                    );
                if (!hasError) {
                    $("html, body").animate(
                        {
                            scrollTop:
                                $(this).find("input[name^=kho_sanpham_shop]").offset().top - 100,
                        },
                        500
                    );
                    hasError = true;
            ...
            if (size && color && gia_moi && !isNaN(gia_moi) && Number(gia_moi) > 0) {
                pl++;
                const jsonStr = `{"ma_sp":"${ma_sp}","color":"${color}","size":"${size}","ten_color":"${ten_color}","ma_mau":"${ma_mau}","ten_size":"${ten_size}","can_nang":"${can_nang}","gia_cu":"${gia_cu}","gia_moi":"${gia_moi}","kho_sanpham_shop":"${kho_sanpham_shop}"}`;
                list_phanloai += pl === 1 ? jsonStr : `,${jsonStr}`;
            }
        ...
        if (tieu_de.length < 4 || tieu_de.length > 120) {
            showErrorMessage(
                "Tiêu đề phải có từ 4 đến 120 ký tự.",
                $("input[name=tieu_de]")
            );
            $("input[name=tieu_de]").focus();
            return;
        ...
        const previewSrc = $("#preview-minhhoa").attr("src");
        const hasFile = document.getElementById("minh_hoa").files.length > 0;
        if (!hasFile && (!previewSrc || previewSrc === "/images/no-images.jpg")) {
            showErrorMessage("Vui lòng chọn hình minh họa.", $("#preview-minhhoa"));
            return;
        }
        ...
        if (!list_phanloai) {
            showErrorMessage(
                "Vui lòng thêm phân loại sản phẩm (kích cỡ, màu sắc...).",
                $(".list_phanloai")
            );
            return;
        }
        ...
        if (chieudai_shop && (isNaN(chieudai_shop) || Number(chieudai_shop) <= 0)) {
            showErrorMessage(
                "Chiều dài phải là số lớn hơn 0.",
                $("input[name=chieudai_shop]")
            );
            $("input[name=chieudai_shop]").focus();
            return;
        }
    
        if (chieurong_shop && (isNaN(chieurong_shop) || Number(chieurong_shop) <= 0)) {
            showErrorMessage(
                "Chiều rộng phải là số lớn hơn 0.",
                $("input[name=chieurong_shop]")
            );
            $("input[name=chieurong_shop]").focus();
            return;
        }
    
        if (chieucao_shop && (isNaN(chieucao_shop) || Number(chieucao_shop) <= 0)) {
            showErrorMessage(
                "Chiều cao phải là số lớn hơn 0.",
                $("input[name=chieucao_shop]")
            );
            $("input[name=chieucao_shop]").focus();
            return;
        }
    
        if (noibatText.length < 10) {
            showErrorMessage(
                "Nội dung nổi bật phải có ít nhất 10 ký tự.",
                $("#noibat").parent()
            );
            tinymce.execCommand("mceFocus", false, "noibat");
            return;
        ...
        if (noibatText.length > 200) {
            showErrorMessage(
                "Nội dung nổi bật không được vượt quá 200 ký tự.",
                $("#noibat").parent()
            );
            tinymce.execCommand("mceFocus", false, "noibat");
            return;
        ...
        const noidungText = stripHtml(noidungRaw); // Chỉ dùng để kiểm tra độ dài, không ảnh hưởng đến dữ liệu gửi đi
        if (noidungText.length < 10) {
            showErrorMessage(
                "Nội dung chi tiết phải có ít nhất 10 ký tự.",
                $("#edit_textarea").parent()
            );
            tinymce.execCommand("mceFocus", false, "edit_textarea");
            return;
        }
    
        if (noidungText.length > 3000) {
            showErrorMessage(
                "Nội dung chi tiết không được vượt quá 3000 ký tự.",
                $("#edit_textarea").parent()
            );
            tinymce.execCommand("mceFocus", false, "edit_textarea");
            return;
        ...
        if (!title) {
            showErrorMessage(
                "Tiêu đề SEO không được để trống.",
                $("input[name=title]")
            );
            $("input[name=title]").focus();
            return;
        ...
        if (title.length > 150) {
            showErrorMessage(
                "Tiêu đề SEO không được vượt quá 150 ký tự.",
                $("input[name=title]")
            );
            $("input[name=title]").focus();
            return;
        ...
        if (description.length > 150) {
            showErrorMessage(
                "Mô tả SEO không được vượt quá 150 ký tự.",
                $("textarea[name=description]")
            );
            $("textarea[name=description]").focus();
            return;
        }
        console.log("noibatRaw trước khi gửi:", noibatRaw); 
        // Tạo FormData và gửi yêu cầu AJAX
        const file_data = $("#minh_hoa").prop("files")[0];
        const form_data = new FormData();
        form_data.append("action", "add_sanpham_ngoai");
        form_data.append("file", file_data);
        ...
        form_data.append("info", list_info);
        form_data.append("phan_loai", list_phanloai);
        form_data.append("noibat", noibatRaw);
        form_data.append("noidung", noidungRaw);
        form_data.append("title", title);
        form_data.append("description", description);
        console.log("noibatRaw trước khi gửi:", noibatRaw); 
        $(".load_overlay").show();
        ...
        $.ajax({
          ...
          data: form_data,
          success: function (kq) {
              console.log("Dữ liệu trả về từ server:", kq); // Debug dữ liệu
              const info = JSON.parse(kq);
              setTimeout(() => $(".load_note").html(info.thongbao), 1000);
              setTimeout(() => {
                  $(".load_process").hide();
    ...
    //     const tieu_de = $("input[name=tieu_de]").val().trim();
    //     const link = $("input[name=link]").val().trim();
    //     const chieudai_shop = $("input[name=chieudai_shop]").val().trim();
    //     const chieurong_shop = $("input[name=chieurong_shop]").val().trim();
    //     const chieucao_shop = $("input[name=chieucao_shop]").val().trim();
    //     const list_photo = [];
    //     $(".list_photo img").each(function () {
    //       list_photo.push($(this).attr("src"));
    //     });
    ...
    //       .get()
    //       .join(",");
    //     const noibat = tinyMCE.get("noibat").getContent().trim();
    //     const noidung = tinyMCE.get("edit_textarea").getContent().trim();
    //     const title = $("input[name=title]").val().trim();
    //     const description = $("textarea[name=description]").val().trim();
    ...
    //       const ten_size = $(this).find("input[name^=size]").val().trim();
    //       const ten_color = $(this).find("input[name^=color]").val().trim();
    //       const can_nang = $(this).find("input[name^=can_nang]").val().trim();
    //       const gia_cu = $(this).find("input[name^=gia_cu]").val().trim();
    //       const gia_moi = $(this).find("input[name^=gia_moi]").val().trim();
    //       const kho_sanpham_shop = $(this)
    //         .find("input[name^=kho_sanpham_shop]")
    //         .val()
    //         .trim();
    
    //       // Kiểm tra size và color riêng biệt
    //       if (!size) {
    //         $(this)
    //           .find(".info_name")
    //           .append(
    ...
    //             {
    //               scrollTop:
    //                 $(this).find("input[name^=gia_moi]").offset().top - 100,
    //             },
    //             500
    //           );
    //           hasError = true;
    ...
    //             {
    //               scrollTop:
    //                 $(this).find("input[name^=can_nang]").offset().top - 100,
    //             },
    //             500
    //           );
    //           hasError = true;
    ...
    //             {
    //               scrollTop:
    //                 $(this).find("input[name^=kho_sanpham_shop]").offset().top -
    //                 100,
    //             },
    //             500
    //           );
    ...
    //     // Hàm hiển thị thông báo lỗi và cuộn đến trường lỗi
    //     const showErrorMessage = (message, element) => {
    //       $(".load_overlay").show();
    //       $(".load_process").fadeIn();
    //       $(".load_note").html(message);
    ...
    //     // Kiểm tra dữ liệu đầu vào
    //     if (tieu_de.length < 4 || tieu_de.length > 255) {
    //       showErrorMessage(
    //         "Tiêu đề phải có từ 4 đến 255 ký tự.",
    //         $("input[name=tieu_de]")
    //       );
    //       $("input[name=tieu_de]").focus();
    //       return;
    //     }
    //     if (
    //       !document.getElementById("minh_hoa").files.length &&
    //       !$("#preview-minhhoa").attr("src")
    //     ) {
    //       showErrorMessage("Vui lòng chọn hình minh họa.", $("#preview-minhhoa"));
    //       return;
    //     }
    ...
    //     if (
    //       chieurong_shop &&
    //       (isNaN(chieurong_shop) || Number(chieurong_shop) <= 0)
    //     ) {
    //       showErrorMessage(
    //         "Chiều rộng phải là số lớn hơn 0.",
    //         $("input[name=chieurong_shop]")
    ...
    //       return;
    //     }
    //     if (noibat.length > 100) {
    //       showErrorMessage(
    //         "Nội dung nổi bật không được vượt quá 100 ký tự.",
    //         $("#noibat").parent()
    //       );
    //       tinymce.execCommand("mceFocus", false, "noibat");
    //       return;
    ...
    //       showErrorMessage(
    //         "Nội dung chi tiết không được vượt quá 1000 ký tự.",
    //         $("#edit_textarea").parent()
    //       );
    //       tinymce.execCommand("mceFocus", false, "edit_textarea");
    //       return;
    ...
    //     if (title.length > 155) {
    //       showErrorMessage(
    //         "Tiêu đề SEO không được vượt quá 155 ký tự.",
    //         $("input[name=title]")
    //       );
    //       $("input[name=title]").focus();
    //       return;
    ...
    //       showErrorMessage(
    //         "Mô tả SEO không được vượt quá 200 ký tự.",
    //         $("textarea[name=description]")
    //       );
    //       $("textarea[name=description]").focus();
    //       return;
    ...
    //     $(this).closest(".li_photo").remove(); // Xóa phần tử li_photo chứa ảnh và nút xóa
    //   });
    ...
    $('button[name=add_sanpham]').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (kq) {
                    ...
                    setTimeout(function () {
                        ...
                        $('.load_overlay').hide();
                        if (info.ok == 1) {
                            window.location.href = '/dropship/list-sanpham';
                        }
                    }, 3000);
                }
     ...
     $('.list_photo').on('click', '.fa-close', function () {
        var $item = $(this).closest('.li_photo');
    ...
    $('button[name=edit_sanpham]').on('click', function () {
        ...
        const id = $('input[name=sp_id]').val();
        const tieu_de = $('input[name=tieu_de]').val();
        const list_photo = [];
        $('.list_photo img').each(function () {
            list_photo.push($(this).attr('src'));
        });
        const anh = list_photo.join(',');
        const minh_hoa = $('#preview-minhhoa').attr('src');
        const title = $('input[name=title]').val();
        const description = $('textarea[name=description]').val();
        const list_cat = $('.li_input input[name^=category]:checked').map(function () {
            return $(this).val();
        ...
        const noidung = tinyMCE.get('edit_textarea').getContent();
        const link = $('input[name=link]').val();
        const link_old = $('input[name=link_old]').val();
        const kich_thuoc =
            $('input[name=chieudai_shop]').val() + ', ' +
            $('input[name=chieurong_shop]').val() + ', ' +
            $('input[name=chieucao_shop]').val();

        // Lấy danh sách phân loại từ .list_phanloai
        let list_phanloai = '';
        let pl = 0;
        $('.list_phanloai .li_phanloai').each(function () {
            id_pl = $(this).attr('pl');
            ...
            kho_sanpham_shop = $(this).find('input[name^=kho_sanpham_shop]').val();
            trongluongtinhship = $(this).find('input[name^=trongluongtinhship]').val();
            pl++;
            if (pl == 1) {
                list_phanloai += '{"id":"' + id_pl + '","ma_sp":"' + ma_sp + '","ma_mau":"' + ma_mau + '","color":"' + color + '","size":"' + size + '","ten_color":"' + ten_color + '","ten_size":"' + ten_size + '","can_nang":"' + can_nang + '","gia_cu":"' + gia_cu + '","gia_moi":"' + gia_moi + '","kho_sanpham_shop":"' + kho_sanpham_shop + '","trongluongtinhship":"' + trongluongtinhship + '"}';
            } else {
                list_phanloai += ',{"id":"' + id_pl + '","ma_sp":"' + ma_sp + '","ma_mau":"' + ma_mau + '","color":"' + color + '","size":"' + size + '","ten_color":"' + ten_color + '","ten_size":"' + ten_size + '","can_nang":"' + can_nang + '","gia_cu":"' + gia_cu + '","gia_moi":"' + gia_moi + '","kho_sanpham_shop":"' + kho_sanpham_shop + '","trongluongtinhship":"' + trongluongtinhship + '"}';
            }
        });
        ...
        if (tieu_de.length < 4) {
            ...
            $('.load_overlay').show();
            $('.load_process').fadeIn();
            setTimeout(() => $('.load_note').html('Vui lòng thêm ít nhất một phân loại'), 500);
            setTimeout(() => {
                $('.load_process').hide();
                $('.load_note').html('Hệ thống đang xử lý');
                $('.load_overlay').hide();
            ...
            form_data.append('anh', anh);
            form_data.append('minh_hoa', minh_hoa);
            form_data.append('kich_thuoc', kich_thuoc);
            form_data.append('file', file_data);
            form_data.append('link', link);
            form_data.append('link_old', link_old);
            form_data.append('category', list_cat);
            ...
            form_data.append('description', description);
            form_data.append('id', id);
            form_data.append('phan_loai', list_phanloai); // Gửi danh sách phân loại dưới dạng JSON
            // console.log(list_phanloai);
            // Hiển thị loading và gửi AJAX
            $('.load_overlay').show();
            $('.load_process').fadeIn();
            $.ajax({
                url: '/dropship/process.php',
                ...
                success: function (kq) {
                    ...
                    setTimeout(() => {
                        ...
                        $('.load_overlay').hide();
                        if (info.ok == 1) {
                            window.location = '/dropship/list-sanpham';
                        }
                    }, 3000);
                },
                error: function () {
...
$('#minh_hoa, #bg_banner').on('change', function(e) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            $(this).closest('.form-group').find('.image-preview img').attr('src', e.target.result);
        }.bind(this);
...
$('#feedbackForm').submit(function(e) {
    ...
    const hiddenFields = ['id', 'shop', 'tieu_de', 'name', 'loai', 'giao_dien', 'description'];
