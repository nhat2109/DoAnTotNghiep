...
$(document).ready(function () {
                ...
                success: function (kq) {
                    ...
                    if (info.ok == 1) {
                        ...
                        if ($('#sub_category_ncc .li_input').length > 0) {
                            $('#sub_category_ncc').append('<hr class="hr_' + id + '">' + info.list);
                        } else {
                ...
                success: function (kq) {
                    ...
                    if (info.ok == 1) {
                        if ($('#sub_sub_category_ncc .li_input').length > 0) {
                            $('#sub_sub_category_ncc').append('<hr class="hr_' + id + ' hr_main_' + main + '">' + info.list);
                        } else {
    ...
    $('button[name=edit_category]').on('click', function () {
        ...
        let cat_id_socdo = $('input[name="category_ncc[]"]:checked').map(function () {
            return this.value;
        }).get();
    ...
    $('body').on('click', 'button[name=button_profile]', function () {
        ...
        if (name.length < 2) {
            errorMessage = 'Vui lòng nhập tên công ty/hộ kinh doanh đầy đủ (ít nhất 2 ký tự)';
    ...
    $('body').on('keyup', '.list_phanloai input[name^=color]', function () {
        var li = $(this);
        var key = li.val();
        var url = li.attr('name') === 'color_shop[]' ? '/ncc/process.php' : '/admincp/process.php';
        var action = li.attr('name') === 'color_shop[]' ? 'goiy_color_ncc' : 'goiy_color';
    ...
    $('.button_add_phanloai').on('click', function () {
        ...
        $('.list_phanloai').append('<div class="li_phanloai" pl="">' +
    ...
    $('body').on('keyup', 'input[name=chieudai_shop], input[name=chieurong_shop], input[name=chieucao_shop], .list_phanloai .li_phanloai input[name^=can_nang]', function () {
        updateTrongLuongTinhShip();
    });

    // Hàm tính và cập nhật trọng lượng tính ship
    function updateTrongLuongTinhShip() {
        var chieudai = parseFloat($('input[name=chieudai_shop]').val().replace(/,/g, '')) || 0;
        var chieurong = parseFloat($('input[name=chieurong_shop]').val().replace(/,/g, '')) || 0;
        var chieucao = parseFloat($('input[name=chieucao_shop]').val().replace(/,/g, '')) || 0;
        var thetich_sp = chieudai * chieurong * chieucao;
        var trongluong_kichthuoc = thetich_sp / 6000;

        $('.list_phanloai .li_phanloai').each(function () {
            var can_nang = parseFloat($(this).find('input[name^=can_nang]').val().replace(/,/g, '')) || 0;
            var can_nang_tinhship = Math.max(can_nang, trongluong_kichthuoc);
            $(this).find('input[name^=trongluongtinhship]').val(format_price(can_nang_tinhship));
        });
    }
    ...
    $('body').on('click', '.list_phanloai .list_goiy .li_goiy', function () {
        ...
        if (source === 'shop') {
            if ($(this).parent().parent().find('input[name="color_shop[]"]').length > 0) {
                $(this).parent().parent().find('input[name="ten_color_shop[]"]').val(text);
                if ($(this).attr('ma_mau') !== undefined) {
                    mm = $(this).attr('ma_mau');
                    $(this).parent().parent().find('input[name="ma_mau_shop[]"]').val(mm);
                }
            } else if ($(this).parent().parent().find('input[name="size_shop[]"]').length > 0) {
                $(this).parent().parent().find('input[name="ten_size_shop[]"]').val(text);
            }
        }
            ...
            } else if ($(this).parent().parent().find('input[name="size[]"]').length > 0) {
    ...
    $('body').on('click', '.list_phanloai .li_phanloai .info_action_copy', function () {
        ...
        $newRow.find('input[name^=size_shop]').val($row.find('input[name^=size_shop]').val());
        $newRow.find('input[name^=size_shop]').attr('giatri', $row.find('input[name^=size_shop]').attr('giatri'));
        $newRow.find('input[name^=ten_size_shop]').val($row.find('input[name^=ten_size_shop]').val());
        $newRow.find('input[name^=color_shop]').val($row.find('input[name^=color_shop]').val());
        $newRow.find('input[name^=color_shop]').attr('giatri', $row.find('input[name^=color_shop]').attr('giatri'));
        $newRow.find('input[name^=ten_color_shop]').val($row.find('input[name^=ten_color_shop]').val());
        $newRow.find('input[name^=ma_mau_shop]').val($row.find('input[name^=ma_mau_shop]').val());
        $newRow.find('input[name^=size]').val($row.find('input[name^=size]').val());
        $newRow.find('input[name^=size]').attr('giatri', $row.find('input[name^=size]').attr('giatri'));
        $newRow.find('input[name^=ten_size]').val($row.find('input[name^=ten_size]').val());
        $newRow.find('input[name^=color]').val($row.find('input[name^=color]').val());
        $newRow.find('input[name^=color]').attr('giatri', $row.find('input[name^=color]').attr('giatri'));
        $newRow.find('input[name^=ten_color]').val($row.find('input[name^=ten_color]').val());
        $newRow.find('input[name^=ma_mau]').val($row.find('input[name^=ma_mau]').val());
        $newRow.find('input[name^=can_nang]').val($row.find('input[name^=can_nang]').val());
        $newRow.find('input[name^=gia_cu]').val($row.find('input[name^=gia_cu]').val());
        $newRow.find('input[name^=gia_moi]').val($row.find('input[name^=gia_moi]').val());
        $newRow.find('input[name^=gia_drop]').val($row.find('input[name^=gia_drop]').val());
        $newRow.find('input[name^=gia_ctv]').val($row.find('input[name^=gia_ctv]').val());
        $newRow.find('input[name^=kho_sanpham_shop]').val($row.find('input[name^=kho_sanpham_shop]').val());
        $newRow.find('input[name^=trongluongtinhship]').val($row.find('input[name^=trongluongtinhship]').val());
    ...
    $('button[name=add_sanpham_ngoai]').on('click', function () {
        ...
        var tong_kho_sanpham = $('.list_phanloai .li_phanloai').toArray().reduce((sum, el) => {
    ...
    $(document).ready(function () {
        ...
        $('.list_phanloai .li_phanloai input[name^=ma]').each(function () {
            if ($(this).val().trim() === '') { // Chỉ gán nếu input đang rỗng
                $(this).val(randomCode);
            }
        });
    });
        ...
        $.ajax({
            ...
            success: function (kq) {
                ...
                if (info.ok == 1) {
                    ...
                    let selectedCategories = $('#checked_cat').data('categories').split(",").map(id => id.trim());
