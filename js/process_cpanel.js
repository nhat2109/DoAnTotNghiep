...
function scrollSmoothToBottom (id) {
  ...
  // http://socmoi.vn/admincp/list-thanhvien
  // $(document).ready(function() {
  //     // Lấy giá trị dropship từ data attribute của tr 
  //     $('tr[data-dropship="1"]').css('background-color', '#FAFF9A');
  // });
  // $(document).ready(function() {
  //     // Xử lý sự kiện khi thay đổi select status
  //     $('#status').on('change', function() {
  //         var selectedStatus = $(this).val();
         
  //         if(selectedStatus === '') {
  //             // Hiện tất cả
  //             $('tr[data-dropship]').show();
  //         } else if(selectedStatus === 'active') {
  //             // Chỉ hiện nhà bán (dropship == 1) 
  //             $('tr[data-dropship]').hide();
  //             $('tr[data-dropship="1"]').show();
  //         } else if(selectedStatus === 'inactive') {
  //             // Chỉ hiện mua hàng (dropship == 0)
  //             $('tr[data-dropship]').hide(); 
  //             $('tr[data-dropship="0"]').show();
  //         }
  //     });
  ...
  // });
  // $(document).ready(function(){
  //     // Xử lý sự kiện khi click nút lọc và thay đổi select
  //     $('#filter_button, #status').on('click change', function() {
  //         var startDate = $('#start_date').val();
  //         var endDate = $('#end_date').val();
  //         var status = $('#status').val();
         
  //         // Lọc theo dropship và thời gian
  //         $('tr[data-dropship]').each(function() {
  //             var dropship = $(this).data('dropship');
  //             var createdDate = $(this).data('created'); // Thêm data-created vào tr
  //             var show = true;
             
  //             // Lọc theo trạng thái
  //             if(status !== '') {
  //                 if(status === 'active' && dropship !== '1') show = false;
  //                 if(status === 'inactive' && dropship !== '0') show = false;
  //             }
  ...
  //huyphuc16/04/2025
function confirm_pheduyet_thuonghieu(action, loai, title, id) {
    $('#title_confirm').html(title);
    $('#button_thuchien').attr('action', action);
    $('#button_thuchien').attr('post_id', id);
    $('#button_thuchien').attr('loai', loai);
  ...
  $(document).ready(function() {
    $('body').on('click','.button_timkiem', function () {
        ...
        else if ($('button[name=timkiem_thanhvien_ncc]').length > 0) {
            action = 'timkiem_thanhvien_ncc';
        }
     ...
     setTimeout(function(){
         $.ajax({
             ...
             success: function(kq) {
                 ...
                 $('.total_mua_seeding_ncc').html(info.total_mua_seeding_ncc);
  ...
  /*        $('.load_overlay').show();
         ...
         $('body').on('click', '.add_hot, .da_themhot', function() {
             ...
             $.ajax({
                 ...
                 success: function(response) {
                     ...
                     if (data.ok == 1) {
                         if (action === 'them_hot') {
                             button.removeClass('add_hot btn btn-success').addClass('da_themhot btn btn-danger').text('Đã thêm HOT');
                         } else {
                             button.removeClass('da_themhot btn btn-danger').addClass('add_hot btn btn-success').text('Thêm HOT');
                         }
                     } else {
                         alert(data.thongbao);
                     }
     ...
     //////////////////////////////huyphuc14/04/2025
    $('.anh_thuong_hieu').click(function () {
        $('#anh_thuonghieu').click();
    });
    $('#anh_thuonghieu').change(function () {
        readURL(this, 'preview-thuonghieu');
    });
     /////////////////////////////
  ...
  /*                    var dulieu={
                 ...
                 $.ajax({
                     ...
                     success: function(kq) {
                         ...
                         $.ajax({
                            ...
                            success:function(kq){
                                var info = JSON.parse(kq);
				$(".congviec_cuanhansu_a .table_nhansu_xemviec tbody").html(info.list_nhansu);
                                $(".box_danhsachnhansu table tbody").html(info.list_admin);
                                $(".box_danhsach_congviec table tbody").html(info.list_dexuat_admin);
                                $(".add_form_dexuat_a table tbody").html(info.list_dexuat_nhanvien);
                            }
                         })
                     }
                 });
     ...
     $('button[name=add_thanhvien_nhom]').on('click', function() {
      ...
      ///////////////////////////// nhà cung cấp
    $('body').on('click', 'button[name=add_goi_seeding_shopee_ncc]', function() {
        tieu_de = $('input[name=tieu_de]').val();
        ...
        if (tieu_de.length < 2) {
            ...
            $('.load_process').fadeIn();
            var form_data = new FormData();
            form_data.append('action', 'add_seeding_shopee_ncc');
            form_data.append('tieu_de', tieu_de);
    ...
    /////////////////////////////
    $('body').on('click', 'button[name=edit_goi_seeding_shopee_ncc]', function() {
        tieu_de = $('input[name=tieu_de]').val();
        gia_cu = $('input[name=gia_cu]').val();
        gia = $('input[name=gia]').val();
        thu_tu = $('input[name=thu_tu]').val();
        ...
        if (tieu_de.length < 2) {
            ...
            $('.load_process').fadeIn();
            var form_data = new FormData();
            form_data.append('action', 'edit_seeding_shopee_ncc');
            form_data.append('tieu_de', tieu_de);
            form_data.append('gia_cu', gia_cu);
            form_data.append('gia', gia);
            form_data.append('uu_dai', uu_dai);
     ...
     $('button[name=edit_goi_seeding_shopee]').on('click', function() {
      ...
      /////////////////////////////
      $('button[name=edit_mua_seeding_ncc]').on('click', function() {
        status = $('select[name=status]').val();
        id=$('input[name=id]').val();
        $('.load_overlay').show();
        $('.load_process').fadeIn();
        var form_data = new FormData();
        form_data.append('action', 'edit_mua_seeding_ncc');
        ...
        $.ajax({
            ...
            success: function(kq) {
                ...
                setTimeout(function() {
                    ...
                    $('.load_overlay').hide();
                    if (info.ok == 1) {
                        window.location.href='/admincp/list-mua-seeding-ncc';
                    } else {
     ...
     $('button[name=button_seeding]').on('click', function() {
      ...
      /////////////////////////////
      $('body').on('click', 'button[name=button_seeding_ncc]', function() {
        time_begin = $('input[name=begin]').val();
        time_end = $('input[name=end]').val();
        if (time_begin.length < 10) {
            $('input[name=begin]').focus();
            ...
            $('.load_process').fadeIn();
            var form_data = new FormData();
            form_data.append('action', 'load_seeding_ncc');
            form_data.append('time_begin', time_begin);
            form_data.append('time_end', time_end);
            $.ajax({
                url: '/admincp/process.php',
    ...
    $('button[name=add_brand]').on('click', function () {
        tieu_de = $('input[name=tieu_de]').val();
        thu_tu = $('input[name=thu_tu]').val();
        link_anh = $('input[name=link_anh]').val();
        trang_thai = $('input[name=hien_thi]').val();
        if (tieu_de.length < 2) {
            $('input[name=tieu_de]').focus();
        } else if (thu_tu == '') {
            $('input[name=thu_tu]').focus();
        } else if (link_anh == '') {
            $('input[name=link_anh]').focus();
        } else {
            $('.load_overlay').show();
            $('.load_process').fadeIn();
            var file_data = $('#anh_thuonghieu').prop('files')[0];
            var form_data = new FormData();
            form_data.append('action', 'add_brand');
            form_data.append('file', file_data);
            form_data.append('tieu_de', tieu_de);
            form_data.append('thu_tu', thu_tu);
            form_data.append('link_anh', link_anh);
            form_data.append('trang_thai', trang_thai);
            $.ajax({
                url: '/admincp/process.php',
                type: 'post',
                cache: false,
    ...
    $('button[name=edit_brand]').on('click', function () {
        ...
        if (tieu_de.length < 2) {
            ...
            var file_data = $('#anh_thuonghieu').prop('files')[0];
            var form_data = new FormData();
            form_data.append('action', 'edit_brand');
            form_data.append('file', file_data);
            form_data.append('tieu_de', tieu_de);
            form_data.append('thu_tu', thu_tu);
            form_data.append('link_anh', link_anh);
  ...
  $(document).on("click", 'button[name="add_giaoviec"]', function () {
   var form_data = new FormData();
   nguoinhan = [];
   $("#nguoinhan_selected .selected_nguoinhan_dathem").each(function () {
     nguoinhan.push($(this).attr("data-id"));
   });
  
   nguoigiamsat = [];
   $("#nguoigiam_sat_selected .selected_giamsat_dathem").each(function () {
     nguoigiamsat.push($(this).attr("data-id"));
   });
  
   phong_ban = [];
   $("#selected_phongban .selected_phongban_dathem").each(function () {
     phong_ban.push($(this).attr("data-id"));
   });
   ten_congviec = $("input[name=ten_congviec]").val();
   let chitiet_congviec = $('textarea[name=chitiet_congviec]').val();
   thoi_han = $('input[name=date_line]').val();
   let file = $("#file_dinhkem")[0].files[0];
   uu_tien = $("#uu_tien").val();
     thoigian_phainhanviec = $('#thoigian_phainhanviec').val();
  ...
  $(document).ready(function(){
   $('button[name=xac_nhan_congviec_duocgiao]').on('click',function(){
...
//   });
  ...
  $(document).ready(function () {
   $('body').on("click",'button[name="add_phongbanmoi"]', function () {
   var form_data = new FormData();
     let ten_phongban = $('input[name=ten_phongban_moi]').val();
     let phongbancha = $('select[name=phongbancha]').val();
     console.log(ten_phongban);
  ...
  $(document).ready(function(){
   ...
   $('body').on("click","button[name=submit_update_tiendo]", function () {
    ...
    $(document).ready(function () {
      $('body').on("click",".hover_skin_tr_vieccuanhanvien", function () {
        id = $(this).attr("data-id");
        $(".value_id_bruh_" + id).css("display", "block");
      });
      $('body').on("click",".close_viec_cua_ban", function () {
        $(".dongthechitietcongviec").css("display", "none");
      });
    });
    $(document).ready(function () {
    ...
    $(document).ready(function () {
      $("body").on("click","button[name=xingiahan]", function () {
        ...
        id = $(this).attr("data-id");
        noidung_xingiahan = $("input[name=xingiahan" + id + "]").val();
        thoihan = $('input[name=date_line'+id+']').val();
    ...
    $(document).ready(function(){
      ...
      $('.hidden_scroll_form_tuchoigiaoviec').css('display','none');
      $('body').on('click','button[name=congviec_nv_d]',function(){
        id = $(this).data('id');
  
        $('.value_b_scrikk_a'+id).show();
      })
      $('body').on('click','button[name=tuchoi_congviec_b]',function(){
        id = $(this).data('id');
        $('.value_id_tuchoixacnhan'+id).show();
      })
      $('body').on('click','button[name=xacnhan_tuchoi_congviec_duocgiao]',function(){
        id = $(this).attr('data-id');
  ...
  $(document).ready(function(){
    ...
    $('body').on('click','button[name=dexuat]',function(){
      ...
      $('body').on('click','button[name=submit_dexuat]',function(){
        ...
        noi_dung = $('input[name=noi_dung_dexuat]').val();
...
$(document).ready(function(){
 $('body').on('click','button[name=themnhanvien_vao_phong_ban_a]', function(){
   $('.danhsachnhansu').css('display','block');
   let id_box = $(this).attr('data-id');

   // Gán sự kiện click cho nút tìm nhân viên
   $(document).off('click', 'button[name=search_nhanvien_button]').on('click', 'button[name=search_nhanvien_button]', function() {
       let id = $(this).attr('data-id'); 
...
$(document).ready(function(){
  $('body').on('click','button[name=giaolai_giaoviec]',function(){
     ...
     $(".thongke,.dexuat,.themphongban,.daotao,.thongke_g,.phongban").css("display", "none");
     ...
     $('button[name=themphongban-form]').off('button[name=themphongban-form]').on('click',function(){
   ...
   let id = $(this).attr('data-id');
   let id_nguoi = $('button[name=search_nhanvien_button][data-id]').map(function() {
   return $(this).attr('data-phongban');
}).get();
id_nguoi.forEach(function(item, index) {
   if (item == id) {
     $('button[name=search_nhanvien_button][data-phongban="' + item + '"]').show();
   }
});
...
$(document).on('click', 'button[name=xoa_dulieu_phongban_dachon]', function() {
   let id = $(this).attr('data-id');
...
id_nguoi.forEach(function(item, index) {
   if (item == id) {
     $('button[name=search_nhanvien_button][data-phongban="' + item + '"]').hide();
   }
});
   ...
   if (currentTarget === 'nguoinhan') {
       $('#nguoinhan_selected').append(`
           <div class="selected_nguoinhan_dathem" data-id="${id_nv}">
               ...
               <button name="xoa_dulieu_nv_dachon" data-id="${id_nv}">X</button>
           </div>
       `);
       ...
       $('#nguoigiam_sat_selected').append(`
           <div class="selected_giamsat_dathem" data-id="${id_nv}">
               <h6>${name_button_nv}</h6>
               <button name="xoa_dulieu_giamsat_dachon" data-id="${id_nv}">X</button>
           </div>
       `);
   }
});
...
$(document).on('click', 'button[name=xoa_dulieu_giamsat_dachon]', function() {
   let id = $(this).attr('data-id');
...
$(document).ready(function(){
  $('body').on('click','button[name=tuchoi_dexuat]',function(){
    id = $(this).attr('data-id');
    $('.value_dexuat_'+id).html('<input type="text" name="noidung_tuchoi_dexuat'+id+'"><button name="xacnhantuchoi_dexuat">Xác nhận từ chối</button><button name="quay_lai_dexuat">Quay lại</button>');
    $('body').on('click','button[name=quay_lai_dexuat]',function(){
      $('.value_dexuat_'+id).html(`<button name="xacnhan_dexuat" data-id="${id}">Xác nhận</button><button name="tuchoi_dexuat" data-id="${id}">Từ chối</button>`);
    });
    $('body').on('click','button[name=xacnhantuchoi_dexuat]',function(){
      noidung_dexuat = $('input[name=noidung_tuchoi_dexuat'+id+']').val();
      var form_data = new FormData();
  ...
  $('body').on('click','button[name=xacnhan_dexuat]',function(){
    ...
    $('body').on('click','button[name=xemchitiet]',function(){
      id = $(this).attr('data-id');
      localStorage.setItem('previousForm', $('.dexuat .list_viec_dagiao').html());
      var form_data = new FormData();
     ...
     $.ajax({
       ...
       success: function (kq) {
        var info = JSON.parse(kq);
          $('.dexuat .list_viec_dagiao .box_danhsachnhansu').html(info);
          $('body').on('click','button[name=tuchoi_dexuat]',function(){
    id = $(this).attr('data-id');
    $('.value_dexuat_'+id).html('<input type="text" name="noidung_tuchoi_dexuat'+id+'"><button name="xacnhantuchoi_dexuat">Xác nhận từ chối</button><button name="quay_lai_dexuat">Quay lại</button>');
    $('body').on('click','button[name=quay_lai_dexuat]',function(){
      $('.value_dexuat_'+id).html(`<button name="xacnhan_dexuat" data-id="${id}">Xác nhận</button><button name="tuchoi_dexuat" data-id="${id}">Từ chối</button><button name="xemchitiet" data-id="${id}">Xem chi tiết</button>`);
    });
    $('body').on('click','button[name=xacnhantuchoi_dexuat]',function(){
      noidung_dexuat = $('input[name=noidung_tuchoi_dexuat'+id+']').val();
      var form_data = new FormData();
  ...
  $('body').on('click','button[name=xacnhan_dexuat]',function(){
    ...
    $('body').on('click','button[name=quay_tro_lai_dexuat]',function(){
      var previousContent = localStorage.getItem('previousForm');
    if (previousContent) {
        $('.dexuat .list_viec_dagiao').html(previousContent);
        $('button[name=tuchoi_dexuat]').on('click',function(){
    id = $(this).attr('data-id');
    $('.value_dexuat_'+id).html('<input type="text" name="noidung_tuchoi_dexuat'+id+'"><button name="xacnhantuchoi_dexuat">Xác nhận từ chối</button><button name="quay_lai_dexuat">Quay lại</button>');
    $('body').on('click','button[name=quay_lai_dexuat]',function(){
      $('.value_dexuat_'+id).html(`<button name="xacnhan_dexuat" data-id="${id}">Xác nhận</button><button name="tuchoi_dexuat" data-id="${id}">Từ chối</button><button name="xemchitiet" data-id="${id}">Xem chi tiết</button>`);
...
//////////////////
$(document).ready(function () {
   let itemsPerPage = 10; // Số thẻ hiển thị mỗi trang
   let currentPage = 1; // Trang hiện tại
   let $items = $(".hover_id_giaoviec"); // Lấy tất cả thẻ
   let filteredItems = $items; // Ban đầu, tất cả thẻ đều hiển thị

   function hienThiTrang(page) {
       $(".hover_id_giaoviec").hide(); // Ẩn tất cả thẻ trước
       let start = (page - 1) * itemsPerPage;
       let end = start + itemsPerPage;

       // Chỉ hiển thị các thẻ trong phạm vi trang hiện tại
       filteredItems.slice(start, end).show();

       // Cập nhật thông tin trang
       $("#thongTinTrang").text(
           `Trang ${page} / ${Math.ceil(filteredItems.length / itemsPerPage)} (${start + 1} - ${Math.min(end, filteredItems.length)} công việc)`
       );

       // Cập nhật lại nút phân trang
       capNhatPhanTrang(page);
   }

   function capNhatPhanTrang(page) {
       $("#phanTrang").empty();
       let totalPages = Math.ceil(filteredItems.length / itemsPerPage);

       // Ẩn nút phân trang nếu số lượng thẻ nhỏ hơn hoặc bằng 10
       ...
       // Nút "Trước"
       if (page > 1) {
           $("#phanTrang").append(`<button onclick="hienThiTrang(${page - 1})">« Trước</button>`);
       }

       // Hiển thị số trang
       for (let i = 1; i <= totalPages; i++) {
           if (i === page) {
               $("#phanTrang").append(`<button style="font-weight:bold;" disabled>${i}</button>`);
           } else {
               $("#phanTrang").append(`<button onclick="hienThiTrang(${i})">${i}</button>`);
           }
       }

       // Nút "Sau"
       if (page < totalPages) {
           $("#phanTrang").append(`<button onclick="hienThiTrang(${page + 1})">Sau »</button>`);
       }
   }

   // Hàm tìm kiếm dựa trên input
   function timKiem(searchTerm) {
       if (searchTerm.trim() === "") {
           filteredItems = $items; // Nếu input rỗng thì hiển thị tất cả
       } else {
           filteredItems = $items.filter(function () {
               return $(this).attr("data-title").toLowerCase().includes(searchTerm.toLowerCase());
           });
       }

       currentPage = 1; // Luôn quay lại trang đầu tiên khi tìm kiếm
       hienThiTrang(currentPage);
   }

   // Lắng nghe sự kiện nhập vào ô tìm kiếm
   $('input[name="form_seach_giaoviec_list"]').on("input", function () {
...
$(document).ready(function(){
       $('button[name=themnguoinhan_taiphongban]').off('button[name=themnguoinhan_taiphongban]').on('click',function(){
           $('.list_danhsachnhansu_form').css('display','block');
           $('button[name=search_nhanvien_button]').on('click',function(){
           id = $(this).attr('data-id');
           name = $(this).attr('data-name');
           $('#nguoinhan_selected_taiphongban').append('<div class="li_nguoinhan" data-id="'+id+'">'+name+'<button class="remove_nguoinhan">X</button></div>');
           $(this).remove();
       ...
       $('button[name=themtruongphong_taiphongban]').off('button[name=themtruongphong_taiphongban]').on('click',function(){
           $('.list_danhsachnhansu_form').css('display','block');
           $('button[name=search_nhanvien_button]').on('click',function(){
           id = $(this).attr('data-id');
           name = $(this).attr('data-name');
           $('#truongphong_selected_taiphongban').html('<div class="li_nguoinhan" data-id="'+id+'">'+name+'<button class="remove_nguoinhan">X</button></div>');
           $('.remove_nguoinhan').on('click',function(){
               $(this).parent().remove();
           })
       })
       ...
       $('button[name=themnvpb]').on('click',function(){
           id = $(this).attr('data-id');
           let nguoinhan = [];
           $('#nv_duoc_chon_vaopb'+id+' .selected_nhanvien_dathem').each(function(){
               nguoinhan.push($(this).attr('data-id'));
           })
   ...
   $(document).ready(function(){
     $('.tr_phongban_box').off('.tr_phongban_box').on('click',function(){
       let id = $(this).attr('data-id');
       $('.chitietphongban_'+id).css('display','block');
     })
     $('.close_add_nv_zo_phongban button').off('.close_add_nv_zo_phongban button').on('click',function(){
       id = $(this).attr('data-id');
...
$(document).ready(function(){
   $('body').on('click','.hover_id_giaoviec', function(){
       let id = $(this).attr('data-id');
       $('.tr_'+id).css('display', 'block');
       $('button[name=close_chitiet_congviec]').off('click').on('click', function(){
...
//////////////////////////
$(document).ready(function () {
   $("button[name=dexuat]").on("click", function () {
     $(".dexuat").css("display", "block");
     $(".btn_remove_giaoviec").removeClass('active');
     $('button[name=dexuat]').addClass('active');

     $(".giaoviec,.thongke,.themphongban,.daotao,.thongke_g,.phongban").css("display", "none");

   });
   ////////////////////////
   $("button[name=phongban]").on("click", function () {
     $(".phongban").css("display", "block");
     $(".btn_remove_giaoviec").removeClass('active');
     $('button[name=phongban]').addClass('active');

     $(".giaoviec,.thongke,.themphongban,.daotao,.thongke_g,.dexuat").css("display", "none");

   });
   ////////////////////////////////
   $("button[name=thongke]").on("click", function () {
     $(".thongke_g").css("display", "block");
     $(".btn_remove_giaoviec").removeClass('active');
     $('button[name=thongke]').addClass('active');
     
     $(".giaoviec,.dexuat,.themphongban,.daotao,.phongban").css("display", "none");
   });
   ////////////////////////////
   
   ////////////////////////////
   $("button[name=giaoviec]").on("click", function () {
     $('button[name=search_nhanvien_button][data-id]').hide();
     $(".giaoviec").css("display", "block");
     $(".thongke,.dexuat,.themphongban,.daotao,.thongke_g,.phongban").css("display", "none");
     $(".btn_remove_giaoviec").removeClass('active');
...
$(document).on('keyup', 'input[name=search_nhanvien]', function() {
   ...
   $("button[name=themphongban]").on("click", function () {
     ...
     $(".giaoviec,.dexuat,.thongke,.daotao,.thongke_g,.phongban").css("display", "none");
   });
   ///////////////////////////////////
   $("button[name=daotao]").on("click", function () {
     $(".daotao").css("display", "block");
     $(".btn_remove_giaoviec").removeClass('active');
     $('button[name=daotao]').addClass('active');
     $(".giaoviec,.dexuat,.themphongban,.thongke,.thongke_g,.phongban").css("display", "none");
   });

   function searchProducts() {
        var selected_ncc = $('#timkiem_ncc').val();
        var selected_status = $('#timkiem_status').val();
        var search = $('.search_sanpham_ncc').val();
    ...
    $('#timkiem_ncc, #timkiem_status').on('change', function() {
        searchProducts();
    });

    $('.search_sanpham_ncc').on('keyup', function(e) {
        if (e.which == 13) {
            searchProducts();
            return;
        }
    ...
    $('.search_sanpham_ncc').on('blur', function() {
        searchProducts();
    });
    // huyphuc14/04/2025
    $('#hien_thi_co').on('change', function () {
        if ($(this).is(':checked')) {
            $('#hien_thi_khong').prop('checked', false);
        }
    });

    $('#hien_thi_khong').on('change', function () {
        if ($(this).is(':checked')) {
            $('#hien_thi_co').prop('checked', false);
        }
    });
    const hienThi = parseInt($('.checkbox_group').data('hien-thi'));

    // Tích checkbox dựa trên giá trị hien_thi
    if (hienThi === 1) {
        $('#hien_thi_co').prop('checked', true);
    } else if (hienThi === 0) {
        $('#hien_thi_khong').prop('checked', true);
    }
    $('.hienthicokhong').each(function() {
        const hienthicokhong = parseInt($(this).data('ht'));

        if (hienthicokhong === 1) {
            $(this).text('Có').addClass('co');
        } else if (hienthicokhong === 0) {
            $(this).text('Không').addClass('khong');
        } else {
            $(this).text('Không xác định').addClass('khong'); // Trường hợp giá trị không hợp lệ
        }
    });
 });