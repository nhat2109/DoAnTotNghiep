<div class="li_sanpham_drop" id="tr_{id}">
	<div class="minh_hoa"><a href="/product/{link}.html" target="_blank"><img src="/thumbnail.php?w=320&img={minh_hoa}" width="200"></a></div>
	<div class="tieu_de"><a href="/product/{link}.html" target="_blank">{tieu_de}</a></div>
	<div class="info_sanpham">
		<div class="left">
			<div class="li_info_left"><span>Giá niêm yết:</span><span>{gia_cu}</span></div>
			<div class="li_info_left"><span>Giá bán gợi ý:</span><span>{gia}</span></div>
			<div class="li_info_left"><span>Bán tối thiểu:</span><span>{drop_min}</span></div>
			<div class="li_info_left"><span>Giá nhập:</span><span>{gia_nhap}</span></div>
			<div class="li_info_left"><span>Kho:</span><span>{kho}</span></div>
		</div>
		<div class="right">
			<div class="list_action">
				<a href="javascript" onclick="check_post({id});" class="check_post bg_green">Đăng bán trên website</a>
				<a href="/dropship/list-share-sanpham?id={id}" class="flex" sp_id="{id}" style="overflow: hidden;"><span>Đăng bán</span><img src="/images/fb_zalo.png"></a>
			</div>
		</div>
	</div>
</div>