...
<tr id="tr_{id}">
	...
	<td style="text-align: center;">
		...
		<a href="/dropship/list-share-sanpham?id={id}" class="flex share_facebook" sp_id="{id}">
			<span>Đăng bán</span>
...
<style>
	...
	/* 1. Hiệu ứng border cho ảnh trong overlay dựa trên trạng thái của nút follow */
	.overlay a img {
	...
	/* Khi nút follow có class "added", áp dụng border cho ảnh */
	.overlay a.added img {
		border-color: #e74c3c;
		/* Màu đỏ hiện đại */
	}

	/* 2. Thiết kế thông báo (toast) hiện đại */
	.toast-container {
		position: fixed;
		top: 20px;
		right: 20px;
	...
	/* Thông báo thêm follow: nền gradient nhẹ nhàng */
	.toast-add {
		background: linear-gradient(135deg, #ff6a6a, #ff3d3d);
	}

	/* Thông báo hủy follow: dùng tông màu trung tính hiện đại */
	.toast-remove {
		background: linear-gradient(135deg, #6c757d, #505050);
	}
