    ...
    $(document).ready(function () {
        ...
        $('#list_product_sub .selected-product').each(function () {
            ...
            var product = {
                ...
                gia_cu: $(this).find('.price-old').text(),
                gia_moi: $(this).find('.price-new').text(),
            ...
            $(this).find('.variant-item').each(function () {
                ...
                var variant = {
                    ...
                    ten_color: $(this).find('span').text().split(' - ')[0],
                    ten_size: $(this).find('span').text().split(' - ')[1],
                    gia_cu: $(this).find('.variant-price .price-old').text().replace('đ', '').replace(/\./g, ''),
                    gia_moi: $(this).find('.variant-price .price-new').text().replace('đ', '').replace(/\./g, ''),
                    stock: parseInt($(this).find('.stock').text().replace('Kho: ', '').replace(' sản phẩm', '')) || 0,
                    quantity: parseInt($(this).find('.quantity-input').val()) || 0,
                    gia_deal: $(this).find('input[name="gia_deal_' + variant_id + '"]').val().replace(/[^0-9]/g, '') || 0
                };
        ...
        // Khôi phục danh sách sản phẩm từ session khi tải trang, nếu không có thì dùng initialProducts
        $.ajax({
            url: '/dropship/process.php',
            type: 'post',
            data: {
                action: 'get_selected_products_edit'
            },
            success: function (response) {
                ...
                if (result.status === 'success' && result.products) {
                    ...
                    // Lưu initialProducts vào session để đồng bộ
                    $.ajax({
                        url: '/dropship/process.php',
                        type: 'post',
                        data: {
                            action: 'save_selected_products_edit',
        ...
        function displaySelectedProducts(products) {
            ...
            products.forEach(function (product) {
                ...
                product.variants.forEach(function (variant) {
                    ...
                    html += '<span class="price-old">' + formatNumberVN(variant.gia_cu) + 'đ</span>';
        ...
        $('button[name="edit_flash_sale"]').click(function (e) {
            ...
            $('#list_product_sub .selected-product').each(function () {
                ...
                $(this).find('.variant-item').each(function () {
                    ...
                    var stock = parseInt($(this).find('.stock').text().replace('Kho: ', '').replace(' sản phẩm', '')) || 0;
                    ...
                    if (quantity > stock) {
                        $('.load_overlay').show();
                        $('.load_process').fadeIn();
                        $('.load_note').html('Số lượng flash sale của biến thể ' + ten_color + ' - ' + ten_size + ' vượt quá số lượng kho (' + stock + ' sản phẩm)!');
                        setTimeout(function () {
                            $('.load_process').hide();
                            $('.load_note').html('Hệ thống đang xử lý');
                            $('.load_overlay').hide();
