...
$hientai = time();
$thongtin_deal = mysqli_query($conn, "SELECT * FROM deal WHERE date_end>='$hientai' AND date_start<='$hientai' AND shop='0' AND loai='flash_sale' ORDER BY id DESC");
while ($r_d = mysqli_fetch_assoc($thongtin_deal)) {
	if ($check->show_thu($r_d['date_start']) != '7' and $check->show_thu($r_d['date_start']) != 'cn') {
...
$banner_page = $class_index->list_banner_page($conn, 'banner_flash_sale');
...
$list_sanpham = $class_index->list_flash_sale($conn, $list_flashsale_id, $list_c, $page, $limit);
$replace = array(
	...
	'footer' => $skin->skin_normal('skin/footer'),
	'footer' => $skin->skin_normal('skin/footer'),
	'title' => 'Sản phẩm flash sale - ' . $index_setting['title'],
...
echo $skin->skin_replace('skin/flash_sale', $replace);
