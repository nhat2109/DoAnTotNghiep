...
if ($color_where != '' or $size_where != '' or $brand_where != '' or $price_where != '') {
	$where = $color_where . " " . $size_where . " " . $brand_where . " " . $price_where . " AND " . $where_tukhoa;
} else {
	$where = $where_tukhoa;
}
if (strlen($where) < 5) {
...
// lịch sử chức năng tìm kiếm nâng cao
// Nhận từ khóa từ request 
if (!empty($_POST['key'])) {
	$tukhoa = trim($_POST['key']);

	// Lấy lịch sử tìm kiếm từ cookie
	$search_history = isset($_COOKIE['search_history']) ? json_decode($_COOKIE['search_history'], true) : [];

	// Kiểm tra nếu từ khóa đã tồn tại thì xóa trước khi thêm vào đầu danh sách
	if (($key = array_search($tukhoa, $search_history)) !== false) {
		unset($search_history[$key]);
	}

	// Thêm từ khóa mới vào đầu danh sách
	array_unshift($search_history, $tukhoa);

	// Giới hạn số lượng từ khóa lưu trữ (10 từ)
	$search_history = array_slice($search_history, 0, 10);

	// Lưu lại vào cookie (thời hạn 30 ngày)
	setcookie('search_history', json_encode($search_history), time() + (30 * 24 * 60 * 60), "/");
}
...
$replace = array(
	...
	'tieu_de' => 'Kết quả tìm kiếm "' . $tukhoa . '"',
