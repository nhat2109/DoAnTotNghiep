...
if (!isset($_COOKIE['emin_id'])) {
    ...
    $thongtin = mysqli_query($conn, "SELECT *, count(*) AS total FROM seo WHERE link='$link' AND loai='baiviet'");
    ...
    if ($r_tt['total'] == 0) {
        if (in_array($duoi, array('jpg', 'jpeg', 'png', 'gif', 'webp')) == true) {
            ...
            mysqli_query($conn, "INSERT INTO post(tieu_de,minh_hoa,cat,link,noidung,title,description,view,date_post)VALUES('$tieu_de','$minh_hoa','$category','$link','$noidung','$title','$description','0'," . time() . ")");
            mysqli_query($conn, "INSERT INTO seo (loai,link)VALUES('baiviet','$link')");
        } else {
