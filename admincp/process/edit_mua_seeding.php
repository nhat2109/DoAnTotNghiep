...
if (!isset($_COOKIE['emin_id'])) {
    ...
    $thongtin_mua=mysqli_query($conn,"SELECT *,count(*) AS total FROM mua_seeding_shopee WHERE id='$id'");
    ...
    if($r_m['total']==0){
        ...
        if($r_m['status']==0){
            if($status==3){
                ...
                mysqli_query($conn,"UPDATE mua_seeding_shopee SET status='$status' WHERE id='$id'");
                $ok=1;
