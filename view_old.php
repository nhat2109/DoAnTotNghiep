...
$thongtin=mysqli_query($conn,"SELECT *,count(*) AS total FROM sanpham WHERE link='$blank' ORDER BY id DESC LIMIT 1");
...
if($total_flash==0){
	...
	if($gio>0){
		...
		if($phut>0){
			$giay=floor($expired - $gio*3600 - $phut*60);
		}else{
			$giay=$expired - $gio*3600;
		}
	}else{
		$phut=floor($expired/60);
		if($phut>0){
			$giay=floor($expired - $phut*60);
		}else{
			$giay=$expired;
		}
	}
...
'list_danhmuc' => $list_danhmuc_top['list_parent'],
	'list_danhmuc_sub' => $list_danhmuc_top['list_sub'],	'box_deal_soc'=>$box_deal_soc,
