<div class="item-popup">
    <div style="width: 55%;" class="text-left">
        <div class="item-image"><a class="product-image" href="/product/{link}.html" title="{tieu_de}"><img alt="{tieu_de}" src="/thumbnail.php?w=320&img={minh_hoa}" width="80"></a></div>
        <div class="item-info">
            <p class="item-name"><a href="/product/{link}.html" title="{tieu_de}">{tieu_de}</a></p>
            <p class="item-remove"><a href="javascript:;" class="remove-item-cart" title="Xóa" data-id="{id}"><i class="fa fa-close"></i> Bỏ sản phẩm</a></p>
            {sanpham_moi}
        </div>
    </div>
    <div style="width: 15%;" class="text-right">
        <div class="item-price"><span class="price">{gia_nhap}₫</span></div>
    </div>
    <div style="width: 15%;" class="text-center">
        <button class="reduced items-count btn-minus" type="button">–</button>
        <input type="text" maxlength="12" min="0" class="input-text number-sidebar" name="quantity" size="4" value="{quantity}">
        <button class="increase items-count btn-plus" type="button">+</button>
    </div>
    <div style="width: 15%;" class="text-right"><span class="cart-price"> <span class="price">{thanhtien}</span> </span></div>
</div>